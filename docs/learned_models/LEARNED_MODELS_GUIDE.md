...
MPPI ROS2는 **11가지 학습/적응 기반 동역학 모델**을 지원합니다:
...
10. **LoRA Dynamics**: Low-Rank Adaptation 기반 효율적 온라인 적응
11. **Conformal Predictor**: 분포-무관 온라인 불확실성 정량화 + CBF 동적 마진
...
- **LoRA 적응**: Low-Rank Adapter로 ~10% 파라미터만 온라인 적응 ([LOTF_GUIDE.md](./LOTF_GUIDE.md) 참조)
- **적응 제어**: EKF / L1 기반 학습 없는 실시간 적응
- **BPTT 궤적 학습**: 미분가능 시뮬레이터 + 궤적 수준 역전파 ([LOTF_GUIDE.md](./LOTF_GUIDE.md) 참조)
- **Conformal Prediction**: CP/ACP 기반 동적 안전 마진으로 모델 불확실성 자동 반영 ([CONFORMAL_CBF_GUIDE.md](../safety/CONFORMAL_CBF_GUIDE.md) 참조)
...
**특징**:
- 사전 학습된 MLP의 각 Linear 층에 Low-Rank Adapter (A, B 행렬) 삽입
- 원본 가중치 동결, LoRA 파라미터(~10%)만 온라인 적응
- MAML과 동일한 `adapt()` 인터페이스 → ResidualDynamics + MPPI에 호환
- Pan et al. (2025) "Learning on the Fly" (UZH) 참조

**장점**:
- MAML 대비 ~90% 적은 학습 파라미터로 유사한 적응 성능
- 원본 가중치 보존 (catastrophic forgetting 방지)
- 메타 LoRA 상태 저장/복원으로 안정적 적응
- Spectral 정규화와 결합하여 외삽 안정성 강화

**단점**:
- rank 선택이 성능에 민감 (너무 낮으면 표현력 부족)
- 사전 학습된 MLP 필요 (from scratch 불가)

**사용 사례**:
- MAML 적응이 느리거나 불안정한 경우
- 제한된 계산 자원에서 온라인 적응
- 6-DOF 모바일 매니퓰레이터 등 고차원 시스템
...
# 온라인 적응 (MAML과 동일 인터페이스)
loss = lora.adapt(states, controls, next_states, dt=0.05, restore=True)

# Residual 모델로 사용
residual = ResidualDynamics(base_model=kinematic, learned_model=lora, use_residual=True)
```

자세한 내용은 [LOTF_GUIDE.md](./LOTF_GUIDE.md)를 참조하세요.

### 11. Conformal Predictor (불확실성 정량화)

**특징**:
- 온라인 적응형 Conformal Prediction으로 모델 예측 불확실성을 정량화
- 분포-무관(distribution-free) 커버리지 보장: P(actual ∈ region) ≥ 1-α
- 표준 CP (quantile 기반) 및 적응형 CP (ACP, 지수 가중 quantile) 지원
- Shield-MPPI의 `safety_margin`을 동적으로 갱신하여 보수성 자동 조절
- Dixit et al. (2024) "ACP + Probabilistic CBF" (arXiv:2407.03569) 참조

**장점**:
- 모델 정확 시: 마진 자동 축소 → 불필요한 보수성 제거 (추적 성능 향상)
- 모델 부정확 시: 마진 자동 확대 → 안전성 향상
- 어떤 예측 모델에도 적용 가능 (NN, GP, Ensemble, MAML 등)
- 오프라인 학습 불필요 — 온라인 비순응 점수 추적만으로 동작

**단점**:
- Cold start 기간(min_samples 미만) 동안 기본 마진 사용
- 매우 빠른 분포 변화에는 적응 지연 (gamma 조절로 완화)
- 예측 함수(prediction_fn)의 편향이 커야 의미 있는 동적 마진 생성

**사용 사례**:
- 학습 모델의 예측 오차가 상황에 따라 크게 변하는 경우
- 고정 안전 마진의 보수성/공격성 trade-off를 자동화하려는 경우
- 동적 장애물/좁은 통로 등 상황에 따라 안전 여유를 조절해야 하는 경우
...
# Conformal Prediction + Shield-MPPI
params = ConformalCBFMPPIParams(
    N=30, dt=0.05, K=1024,
    cbf_obstacles=obstacles,
    cbf_safety_margin=0.02,    # Cold start 마진 (CP가 자동 대체)
    cp_alpha=0.1,              # 실패율 (0.1 → 90% 커버리지)
    cp_gamma=0.95,             # ACP 감쇠 (1.0=표준, <1.0=적응형)
    cp_margin_min=0.005,       # 최소 마진 (m)
    cp_margin_max=0.5,         # 최대 마진 (m)
)

# 학습 모델 예측 함수 연결 (선택사항 — None이면 model.step 사용)
pred_fn = lambda s, u: learned_model.step(s, u, dt)
controller = ConformalCBFMPPIController(model, params, prediction_fn=pred_fn)

# 매 스텝 자동 동작:
# 1. 이전 예측 vs 실제 상태로 CP 비순응 점수 업데이트
# 2. quantile(scores, 1-α) → 동적 마진 계산
# 3. cbf_cost.safety_margin = cp_margin (자동 갱신)
# 4. Shield-MPPI 제어 계산 (갱신된 마진 사용)
control, info = controller.compute_control(state, reference_trajectory)
print(f"CP margin: {info['cp_margin']:.3f}m, Coverage: {info['cp_empirical_coverage']:.1%}")
```

자세한 내용은 [CONFORMAL_CBF_GUIDE.md](../safety/CONFORMAL_CBF_GUIDE.md)를 참조하세요.
...
| **LoRA** | **20~50 (적응)** | **~5분 (사전학습)** | **~10ms (적응)** | **없음** |
| **Conformal Predictor** | **없음** | **없음** | **~0.01ms** | **분포-무관 마진** |
...
#### LoRA Dynamics를 선택하세요:
- ✅ MAML 적응이 느리거나 불안정한 경우
- ✅ 제한된 계산 자원에서 온라인 적응 (~10% 파라미터)
- ✅ 사전 학습된 MLP가 이미 존재
- ✅ catastrophic forgetting 없는 안전한 적응
- ❌ 사전 학습된 모델이 없음 (from scratch 불가)
- ❌ 불확실성 정량화 필요 (ALPaCA/GP가 적합)
...
class LoRADynamics(NeuralDynamics):
    def __init__(
        ...
        model_path: Optional[str] = None,
        device: str = "cpu",
        lora_rank: int = 4,        # Low-rank 차원 (작을수록 파라미터 적음)
        lora_alpha: float = 1.0,   # LoRA 스케일링 팩터
    ...
    def adapt(
        ...
        next_states: np.ndarray,  # (M, nx)
        dt: float,
        restore: bool = True,     # True → 메타 LoRA 복원 후 적응
        sample_weights: Optional[np.ndarray] = None,
        temporal_decay: Optional[float] = None,
    ) -> float:
        """Few-shot 적응 (LoRA 파라미터만 SGD) → 최종 loss 반환"""
...
### LotF (Learning on the Fly) 벤치마크

6-DOF 모바일 매니퓰레이터에 대해 LoRA, BPTT, Spectral 정규화, NN-Policy 등 LotF 기법을 8-Way 비교합니다.
...
# 실시간 시각화
PYTHONPATH=. python examples/comparison/lotf_benchmark.py --live
...
8-Way 비교 대상:
1. Kinematic (기준선), 2. Res-NN (MSE),
3. Res-NN+Spectral (MSE+σ_max), 4. Res-BPTT (궤적 역전파),
5. Res-LoRA (온라인 적응), 6. Res-MAML (메타 적응),
7. NN-Policy (BC+BPTT, MPPI 없이 직접 제어), 8. Oracle
...
- Pan et al. (2025) - "Learning on the Fly: Rapid Policy Adaptation via Differentiable Simulation" (LotF)
