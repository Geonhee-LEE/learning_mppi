...
27. [SVG-MPPI (Stein Variational Guided MPPI)](#27-svg-mppi-stein-variational-guided-mppi)
28. [pi-MPPI (Projection-based MPPI)](#28-pi-mppi-projection-based-mppi)
29. [dsMPPI (Deterministic Sampling MPPI)](#29-dsmppi-deterministic-sampling-mppi)
30. [DRPA-MPPI (Dynamic Repulsive Potential Augmented MPPI)](#30-drpa-mppi-dynamic-repulsive-potential-augmented-mppi)
31. [CSC-MPPI (Constrained Sampling Cluster MPPI)](#31-csc-mppi-constrained-sampling-cluster-mppi)
32. [T-MPPI (Transformer-based Initialization MPPI)](#32-t-mppi-transformer-based-initialization-mppi)
33. [F-MPPI (Feedback Reuse MPPI)](#33-f-mppi-feedback-reuse-mppi)
...
## 27. SVG-MPPI (Stein Variational Guided MPPI)

**Reference**: Honda et al., ICRA 2024, arXiv:2309.11040

### 27.1 핵심 아이디어

표준 MPPI는 **등방 가우시안 노이즈**에서 샘플을 생성하므로, 비용 지형의
고밀도 영역(최적 해 근방)에 샘플이 집중되지 않는다. 다중 모달 비용 지형에서는
샘플 효율이 특히 낮아진다.

**SVG-MPPI**는 **Stein Variational Gradient Descent (SVGD)**를 활용하여
소수의 **가이드 파티클**을 비용 지형의 최적 모드로 이동시킨 후,
이 파티클 주변에서 MPPI 샘플을 생성한다. 학습 없이 커널 기반 추론만으로
샘플 분포를 적응적으로 조정하는 것이 핵심이다.
...
```
파티클 집합 {x_i}_{i=1}^G 가 목표 분포 p(x)를 근사하도록 이동:

φ*(x) = (1/G) Σ_{j=1}^G [ k(x_j, x) ∇_{x_j} log p(x_j) + ∇_{x_j} k(x_j, x) ]
                            \_________________________/   \____________________/
                                 exploitation 항               exploration 항
                           (고밀도 영역으로 이동)         (파티클 간 반발력)

x_i ← x_i + η · φ*(x_i)
```

여기서:
- `k(x, x')`: 양정치 커널 함수 (RBF 커널 사용)
- `∇log p(x)`: 목표 분포의 score function
- `η`: SVGD 스텝 크기

**RBF 커널 + Median Bandwidth Heuristic**:

```
k(x, x') = exp(-||x - x'||² / (2h²))

h = median({||x_i - x_j||²}_{i≠j}) / log(G+1)    (median heuristic)
```

median heuristic은 파티클 간 거리 분포에 따라 커널 대역폭을 자동 조정한다.

**SPSA(Simultaneous Perturbation Stochastic Approximation) Gradient**:

전체 제어 시퀀스의 기울기를 효율적으로 추정한다.
N×nu 차원의 기울기를 단 2회 rollout으로 근사:

```
∇_V S(V) ≈ [S(V + c·Δ) - S(V - c·Δ)] / (2c) · Δ^{-1}

여기서:
  S(V): 제어 시퀀스 V의 비용
  Δ: Rademacher 랜덤 벡터 (±1)
  c: 섭동 크기
  Δ^{-1}: 요소별 역수 (1/Δ_i)
```

SPSA는 유한 차분법(O(2·N·nu) rollouts) 대비 O(2) rollouts로 동일 차원의
기울기를 추정하므로, 고차원 제어에서 매우 효율적이다.

### 27.3 알고리즘

```
매 스텝:
  [Phase 1: SVGD Guide Particle Update]
  1. G개 가이드 파티클 초기화: {V_g}_{g=1}^G ← 이전 해 + 작은 노이즈
  2. for s = 1 to n_svgd_steps:
     a. 각 파티클의 SPSA gradient 계산:
        ∇_g ≈ [S(V_g + cΔ) - S(V_g - cΔ)] / (2c·Δ)
     b. 파티클 간 커널 행렬 K_{ij} = k(V_i, V_j) 계산
     c. Median bandwidth h 갱신
     d. SVGD 업데이트:
        V_g ← V_g + η · (1/G) Σ_j [K_{gj} · (-∇_j/λ) + ∇_{V_j} K_{gj}]

  [Phase 2: Guided MPPI Sampling]
  3. 각 가이드 파티클 주변에서 K/G개 노이즈 샘플 생성:
     ε_{g,k} ~ N(V_g - U_nom, σ²I)
  4. K개 후보 시퀀스 rollout: V_k = U_nom + ε_k
  5. 비용 계산: S_k = Σ c(x_t, u_t)
  6. MPPI 가중 평균: U ← U_nom + Σ ω_k ε_k

  [Phase 3: Blending]
  7. 최종 출력 = α · U_svgd_best + (1-α) · U_mppi
```
...
```python
# 컨트롤러
SVGMPPIController(MPPIController)
  |-- compute_control(): SVGD + guided MPPI + blending
  |-- _svgd_update(): G 파티클을 n_svgd_steps 만큼 SVGD 이동
  |-- _spsa_gradient(): 2-rollout SPSA gradient 추정
  |-- _compute_kernel_matrix(): RBF 커널 + median bandwidth
  |-- _generate_guided_samples(): 가이드 파티클 주변 샘플링
  +-- _blend_solutions(): SVGD best + MPPI 결과 혼합

# 파라미터
SVGMPPIParams(MPPIParams)
  |-- n_guide_particles: 64      # 가이드 파티클 수 G
  |-- n_svgd_steps: 3            # SVGD 반복 횟수
  |-- svgd_step_size: 0.1        # SVGD 학습률 η
  |-- temperature_svgd: 1.0      # SVGD 온도 (score 스케일링)
  |-- blend_ratio: 0.3           # α (SVGD vs MPPI 혼합 비율)
  |-- spsa_perturbation: 0.1     # SPSA 섭동 크기 c
  +-- use_median_bandwidth: True  # median heuristic 활성화
```

### 27.5 기존 변형 대비

| 특성 | Vanilla MPPI | SVMPC | SG-MPPI | SVG-MPPI |
|------|-------------|-------|---------|----------|
| 샘플 분포 | 등방 가우시안 | SVGD 파티클 | score 편향 가우시안 | SVGD 가이드 + 가우시안 |
| 학습 필요 | 없음 | 없음 | score network | 없음 (커널만) |
| Gradient | 불필요 | 커널 gradient | DSM 학습 | SPSA (2 rollouts) |
| 다중 모달 | 불가 | 가능 (높은 비용) | 부분적 | 가능 (효율적) |
| 계산 복잡도 | O(K) | O(K²) | O(K + 학습) | O(K + 2G·M) |
| Cold start | 즉시 | 즉시 | 학습 후 | 즉시 |

SVMPC는 모든 K 파티클을 SVGD로 이동하여 O(K²)인 반면,
SVG-MPPI는 소수 G(<<K)만 SVGD로 이동하고 나머지는 가이드 샘플링하여
O(K + 2G·M)으로 효율적이다. SG-MPPI는 score network 학습이 필요하지만,
SVG-MPPI는 학습 없이 SPSA + 커널만으로 동작한다.

---

## 28. pi-MPPI (Projection-based MPPI)

**Reference**: Andrejev et al., RA-L 2025, arXiv:2504.10962

### 28.1 핵심 아이디어

표준 MPPI의 가우시안 샘플링은 **제어 입력의 시간적 매끄러움을 보장하지 않는다**.
인접 시간 스텝 간의 제어 변화가 급격하면, 실제 로봇에서 **jerk(가속도 변화율)**와
**snap(jerk 변화율)**이 과도하게 발생하여 하드웨어 한계를 초과할 수 있다.

LP-MPPI나 Smooth-MPPI는 **소프트 제약** (필터링, 비용 항)으로 매끄러움을 유도하지만,
하드 바운드를 보장하지 않는다.

**pi-MPPI**는 **QP(Quadratic Programming) 또는 순차 클리핑 투영**으로
모든 샘플 궤적에 jerk/snap 하드 제약을 **사전 투영(projection)**하여,
물리적으로 실현 불가능한 샘플이 롤아웃에 진입하지 않도록 보장한다.
...
```
Jerk:  j(t) = (v(t) - v(t-1)) / dt           → |j(t)| ≤ j_max  ∀t
Snap:  s(t) = (j(t) - j(t-1)) / dt           → |s(t)| ≤ s_max  ∀t
```

**QP 투영 (정확한 방법)**:

```
min_{V_proj}  ||V_proj - V_orig||²

s.t.  |(V_proj[t] - V_proj[t-1]) / dt| ≤ j_max      (jerk 제약)
      |(V_proj[t] - 2V_proj[t-1] + V_proj[t-2]) / dt²| ≤ s_max  (snap 제약)
```

이는 볼록 QP이므로 전역 최적해가 존재하며, 원래 샘플에 가장 가까운
실현 가능한 제어 시퀀스를 반환한다. scipy SLSQP로 풀 수 있다.
...
for t = 1 to N-1:
  delta = V[t] - V[t-1]
  if |delta/dt| > j_max:
    V[t] = V[t-1] + sign(delta) · j_max · dt    (jerk 클리핑)

for t = 2 to N-1:
  jerk_t = V[t] - 2V[t-1] + V[t-2]
  if |jerk_t/dt²| > s_max:
    V[t] = 2V[t-1] - V[t-2] + sign(jerk_t) · s_max · dt²  (snap 클리핑)
```

순차 클리핑은 QP 대비 ~100배 빠르지만, 동시 jerk+snap 제약 하에서
최적성을 보장하지 않는다.
...
```
매 스텝:
  1. 표준 MPPI 노이즈 샘플링: ε_k ~ N(0, σ²I), k=1,...,K
  2. 후보 시퀀스 생성: V_k = U_nom + ε_k
  3. [투영 단계] 각 V_k에 대해:
     a. clip 방법: 순차 jerk → snap 클리핑
     b. qp 방법:   SLSQP QP 풀기
     → V_k^proj (제약 만족 시퀀스)
  4. 투영된 시퀀스로 rollout: x_{t+1} = f(x_t, V_k^proj[t])
  5. 비용 계산: S_k = Σ c(x_t, V_k^proj[t])
  6. MPPI 가중 평균: U ← U_nom + Σ ω_k (V_k^proj - U_nom)
  7. 최종 U도 투영 적용 (보장)
```
...
```python
# 컨트롤러
ProjectionMPPIController(MPPIController)
  |-- compute_control(): 샘플링 → 투영 → rollout → 가중 평균
  |-- _project_sequences(): K개 시퀀스 일괄 투영
  |-- _clip_projection(): 순차 jerk/snap 클리핑 (벡터화)
  |-- _qp_projection(): scipy SLSQP 최적화
  |-- _compute_jerk(): 시퀀스의 jerk 계산
  +-- _compute_snap(): 시퀀스의 snap 계산

# 파라미터
ProjectionMPPIParams(MPPIParams)
  |-- projection_method: "clip"  # "clip" 또는 "qp"
  |-- jerk_max: 5.0              # 최대 jerk (rad/s² 또는 m/s²)
  |-- snap_max: 10.0             # 최대 snap
  |-- use_jerk_constraint: True  # jerk 제약 활성화
  |-- use_snap_constraint: True  # snap 제약 활성화
  +-- project_final: True        # 최종 시퀀스에도 투영 적용
```

### 28.5 기존 변형 대비

| 특성 | Vanilla MPPI | LP-MPPI | Smooth MPPI | pi-MPPI |
|------|-------------|---------|-------------|---------|
| 매끄러움 보장 | 없음 | 소프트 (LPF) | 소프트 (비용 항) | **하드 (투영)** |
| Jerk 바운드 | 비보장 | 간접적 감소 | 비용 기반 감소 | **|j|≤j_max 보장** |
| 방법론 | 기준 | Butterworth 필터 | ΔU + jerk cost | QP/clip 투영 |
| 추가 계산 | 없음 | sosfilt (저비용) | 비용 항 | 투영 (clip: 저, qp: 중) |
| 물리적 실현성 | 비보장 | 높은 확률 | 높은 확률 | **100% 보장** |
| MSSD 개선 | 기준 | ~15배 | ~5배 | **~15-60배** |

pi-MPPI는 **제약 만족 보장**이 핵심 차별점이다. LP-MPPI는 주파수 도메인에서
부드럽게 하지만 하드 바운드를 보장하지 않고, Smooth-MPPI는 비용 함수에
jerk 페널티를 추가하지만 소프트 제약이다. pi-MPPI만이 QP 투영으로
모든 샘플이 jerk/snap 한계 내에 있음을 수학적으로 보장한다.

---

## 29. dsMPPI (Deterministic Sampling MPPI)

**Reference**: Walker et al., arXiv:2601.03893, 2026

### 29.1 핵심 아이디어

기존 27종 이상의 MPPI 변형은 모두 **확률적(stochastic) 샘플링**에 기반한다.
가우시안 노이즈 ε ~ N(0, σ²I)는 구현이 간단하지만, 유한 샘플 K에서
**제어 공간을 균일하게 커버하지 못한다** (확률적 공간 커버리지 편차).

**dsMPPI**는 **결정론적 시퀀스** (Halton, Sobol, sigma points)를 사용하여
제어 공간을 균일하게 커버한다. 확률적 샘플링 대비 적은 K로 동등한 커버리지를
달성하며, **CEM(Cross-Entropy Method) 반복**으로 분포를 점진적으로 정제한다.

### 29.2 수학적 배경

**Quasi-Monte Carlo (QMC) 변환**:

```
1. 단위 초입방체에서 결정론적 점 생성: u_i ∈ [0,1]^d  (QMC 시퀀스)
2. PPF(Percent-Point Function, 역 CDF) 변환으로 가우시안 매핑:
   q_i = Φ^{-1}(u_i)   where Φ = standard normal CDF

   결과: q_i ~ N(0, I) 와 동일한 주변 분포이지만,
         공간 커버리지가 확률적 샘플 대비 우수
```

**Halton 시퀀스**: 서로소 소수 기저(2, 3, 5, ...)의 Van der Corput 시퀀스 직곱:

```
Base-2 Van der Corput: 1/2, 1/4, 3/4, 1/8, 5/8, 3/8, 7/8, ...
Base-3 Van der Corput: 1/3, 2/3, 1/9, 4/9, 7/9, 2/9, 5/9, ...

d차원 Halton: H_i = (vdc_2(i), vdc_3(i), vdc_5(i), ..., vdc_{p_d}(i))
```

**Sobol 시퀀스**: 방향 수(direction numbers) 기반 이진 시퀀스로,
Halton 대비 고차원에서 더 균일한 분포를 제공한다.

**Sigma Points**: Unscented Transform에서 유래한 2d+1개 결정론적 점:

```
x_0 = μ                              (가중치 w_0 = κ/(d+κ))
x_i = μ + √((d+κ)Σ)_i    i=1,...,d  (가중치 w_i = 1/(2(d+κ)))
x_{i+d} = μ - √((d+κ)Σ)_i           (가중치 w_{i+d} = 1/(2(d+κ)))
```

**CEM (Cross-Entropy Method) 반복**:

```
for iter = 1 to M:
  1. 현재 분포 N(μ, diag(σ²))에서 K개 결정론적 샘플 생성
  2. 비용 계산 → 상위 elite_ratio 선택
  3. EMA 업데이트:
     μ_new = (1-α)·μ + α·mean(elite)
     σ_new = (1-β)·σ + β·std(elite)
```
...
```
초기화:
  - QMC 시퀀스 사전 생성: H = {h_1, ..., h_K} ∈ [0,1]^{K×N×nu}
  - PPF 변환: Q = Φ^{-1}(H)  (결정론적 "노이즈")

매 스텝:
  1. 현재 분포: μ = U_nom, σ = σ_init

  [CEM 반복]
  for iter = 1 to n_cem_iters:
    2. 스케일링된 샘플: V_k = μ + σ ⊙ Q_k    (k=1,...,K)
    3. Rollout: x_{t+1} = f(x_t, V_k[t])
    4. 비용 계산: S_k = Σ c(x_t, V_k[t])
    5. Elite 선택: top-E 인덱스 (E = K × elite_ratio)
    6. 분포 갱신:
       μ ← (1-α_μ)·μ + α_μ·mean(V_elite)
       σ ← (1-α_σ)·σ + α_σ·std(V_elite)

  [최종 MPPI 가중 평균]
  7. 최종 반복의 비용으로 가중치 계산: ω_k = softmax(-S_k / λ)
  8. U ← Σ ω_k · V_k
```
...
```python
# 컨트롤러
DeterministicMPPIController(MPPIController)
  |-- compute_control(): QMC 샘플 → CEM 반복 → MPPI 가중 평균
  |-- _generate_qmc_samples(): Halton/Sobol/sigma points 생성
  |-- _ppf_transform(): 균일 → 가우시안 역 CDF 변환
  |-- _cem_iteration(): elite 선택 → μ, σ 갱신
  +-- _compute_weights(): 표준 MPPI softmax 가중치

# 파라미터
DeterministicMPPIParams(MPPIParams)
  |-- sampling_method: "halton"   # "halton", "sobol", "sigma_points"
  |-- n_cem_iters: 3              # CEM 반복 횟수
  |-- elite_ratio: 0.1            # 상위 비율 (10%)
  |-- cem_alpha_mu: 0.5           # 평균 EMA 학습률
  |-- cem_alpha_sigma: 0.5        # 표준편차 EMA 학습률
  |-- sigma_min: 0.01             # 최소 표준편차 (수렴 방지)
  +-- use_scrambled: True         # 스크램블 시퀀스 (랜덤화)
```

### 29.5 기존 변형 대비

| 특성 | Vanilla MPPI | CMA-MPPI | DIAL-MPPI | dsMPPI |
|------|-------------|----------|-----------|--------|
| 샘플링 | 확률적 가우시안 | 확률적 (적응 Σ) | 확률적 (어닐링) | **결정론적 QMC** |
| 공간 커버리지 | O(1/√K) | O(1/√K) | O(1/√K) | **O(1/K)** (QMC) |
| 반복 방법 | 단일 | 공분산 적응 | 온도 감쇠 | CEM (μ, σ 적응) |
| 재현성 | 비재현 | 비재현 | 비재현 | **완전 재현 가능** |
| 최소 K | ~512 | ~256 | ~512 | **~64** (균일 커버) |
| 비용 계산 | K rollouts | M×K rollouts | M×K rollouts | M×K rollouts |

dsMPPI의 핵심 장점은 **적은 샘플(K=64)로도 등가 성능 달성**이다.
QMC의 불일치도(discrepancy)가 O(log^d(K)/K)로, 확률적 샘플링의 O(1/√K)보다
빠르게 감소한다. 특히 **임베디드/저전력 환경**에서 K를 줄여야 할 때
결정론적 샘플링이 확률적 대비 유리하다. 또한 완전 재현 가능하여 디버깅이 용이하다.

---

## 30. DRPA-MPPI (Dynamic Repulsive Potential Augmented MPPI)

**Reference**: Fuke et al., arXiv:2503.20134, 2025

### 30.1 핵심 아이디어

MPPI는 샘플 기반 최적화이므로, **local minima**(지역 최소)에 빠질 수 있다.
특히 큰 장애물 뒤에 목표가 있는 경우, 비용 지형에 함정이 형성되어
로봇이 정체하거나 진동하는 현상이 발생한다.

기존 해결법들은 다중 모달 샘플링(Flow, SVG, Biased)이나 반복 정제(DIAL, CMA)에
의존하지만, **학습이 필요하거나 계산 비용이 높다**.

**DRPA-MPPI**는 **local minima 감지 → 반발 포텐셜 자동 추가 → 탈출 → 복귀**의
3-phase 전략으로, 학습 없이 즉시 작동하며 낮은 추가 비용으로 탈출한다.
...
```
F_rep(x, x_obs) = { η · (1/d(x) - 1/d_0)²    if d(x) < d_0
                   { 0                           otherwise

여기서:
  d(x) = ||x - x_obs|| - r_obs    (장애물 표면까지 거리)
  d_0:  반발 영향 반경
  η:    반발 강도 계수
```
...
```
정체 지표 = ||x_T - x_0|| / (v_max · T)

if 정체 지표 < stall_threshold:
  → local minima 감지 (T 스텝 동안 거의 이동하지 않음)
```

보조 지표:
- 비용 감소율 정체: |S(t) - S(t-W)| / S(t-W) < δ
- 궤적 분산 수축: var(x_T) < ε
...
Phase 2: ESCAPE
  → 비용에 반발 포텐셜 추가: S_eff = S_orig + w_rep · Σ F_rep
  → 탐색 노이즈 부스트: σ_eff = σ · (1 + noise_boost)
  → 장애물로부터 안전 거리 확보 시 Phase 3 전환

Phase 3: RECOVERY
  → 반발 포텐셜 점진적 감쇠: w_rep ← w_rep · decay_rate
  → 노이즈 원복: σ_eff → σ
  → w_rep < ε 또는 타임아웃 시 Phase 1 복귀
```

### 30.3 알고리즘

```
상태: phase ∈ {NORMAL, ESCAPE, RECOVERY}
     stall_counter, escape_target, rep_weight

매 스텝:
  1. 정체 감지:
     if ||x_current - x_prev_window|| < stall_threshold × window:
       stall_counter += 1
     else:
       stall_counter = 0

  2. Phase 전환:
     if phase == NORMAL and stall_counter > stall_patience:
       phase ← ESCAPE
       가장 가까운 장애물 기록 → escape_target
       rep_weight ← η_init

     if phase == ESCAPE and d(x, escape_target) > safe_distance:
       phase ← RECOVERY

     if phase == RECOVERY and rep_weight < ε:
       phase ← NORMAL

  3. 비용 수정:
     if phase == ESCAPE or phase == RECOVERY:
       for each 장애물 obs:
         S_k += rep_weight · F_rep(x_k, obs)
       σ_eff = σ · (1 + noise_boost · I[phase==ESCAPE])

  4. 표준 MPPI 실행 (수정된 비용 + σ_eff):
     ε_k ~ N(0, σ_eff²I)
     V_k = U + ε_k → rollout → S_k → ω_k → U 갱신

  5. 감쇠 (RECOVERY):
     if phase == RECOVERY:
       rep_weight ← rep_weight · decay_rate
```
...
```python
# 컨트롤러
DRPAMPPIController(MPPIController)
  |-- compute_control(): 정체 감지 → phase 관리 → 수정 비용 MPPI
  |-- _detect_stall(): 정체 지표 계산 + stall_counter 갱신
  |-- _update_phase(): NORMAL ↔ ESCAPE ↔ RECOVERY 전환
  |-- _compute_repulsive_cost(): 반발 포텐셜 벡터화 계산
  |-- _boost_noise(): ESCAPE 모드 σ 증가
  +-- get_phase_statistics(): 현재 phase, 반발 강도, 정체 지표

# 파라미터
DRPAMPPIParams(MPPIParams)
  |-- drpa_obstacles: []           # [(x, y, r), ...] 장애물 목록
  |-- repulsive_eta: 5.0           # 반발 강도 η
  |-- repulsive_d0: 1.5            # 반발 영향 반경 d_0
  |-- stall_threshold: 0.05        # 정체 감지 임계값
  |-- stall_patience: 10           # 정체 인내 횟수
  |-- stall_window: 5              # 정체 판단 윈도우
  |-- noise_boost: 2.0             # ESCAPE 모드 σ 증폭률
  |-- decay_rate: 0.9              # RECOVERY 반발 감쇠율
  |-- safe_distance: 0.5           # 탈출 판정 안전 거리
  +-- max_escape_steps: 50         # 최대 탈출 스텝 (타임아웃)
```

### 30.5 기존 변형 대비

| 특성 | Vanilla MPPI | Biased-MPPI | DIAL-MPPI | DBaS-MPPI | DRPA-MPPI |
|------|-------------|-------------|-----------|-----------|-----------|
| Local minima 대응 | 없음 | 정책 다양성 | 온도 어닐링 | barrier 탐색 | **반발 포텐셜** |
| 학습 필요 | 없음 | 정책 설계 | 없음 | 없음 | **없음** |
| 상태 머신 | 없음 | 없음 | 없음 | 없음 | **3-phase** |
| 동적 비용 수정 | 없음 | 없음 | 보상 정규화 | barrier cost | **반발 포텐셜 추가** |
| 추가 계산 | 없음 | 정책 rollout | M×K | barrier 계산 | 정체 감지 + F_rep |
| 탈출 후 복귀 | 해당 없음 | 해당 없음 | 해당 없음 | 해당 없음 | **자동 감쇠 복귀** |

DRPA-MPPI의 고유 특징은 **3-phase 상태 머신**에 의한 적응적 행동이다.
NORMAL에서는 표준 MPPI와 동일하므로 오버헤드가 없고, local minima를
감지한 경우에만 반발 포텐셜을 활성화한다. Biased-MPPI는 항시 다중 정책을
유지하고, DIAL은 항시 반복을 수행하지만, DRPA는 **필요할 때만 개입**한다.

---

## 31. CSC-MPPI (Constrained Sampling Cluster MPPI)

**Reference**: arXiv:2506.16386, 2025

### 31.1 핵심 아이디어

표준 MPPI의 **가중 평균**은 모든 샘플의 선형 결합이므로,
개별 샘플이 제약을 만족하더라도 **평균 궤적이 제약을 위반**할 수 있다.
이를 "평균의 비실현가능성(infeasibility of the mean)" 문제라 한다.

```
예: 장애물 양쪽에 좋은 궤적이 각각 있을 때,
    가중 평균은 장애물을 관통하는 궤적이 될 수 있음
```

**CSC-MPPI**는 두 가지 메커니즘으로 이를 해결한다:
1. **Primal-Dual 제약 투영**: 라그랑주 승수로 제약을 명시적으로 처리
2. **DBSCAN 클러스터링**: 궤적들을 클러스터로 그룹화하고,
   가중 평균 대신 **최적 클러스터의 대표 궤적**을 선택
...
라그랑지안: L(V, λ) = C(V) + Σ_i λ_i · g_i(V)

Primal 업데이트: V ← V - α_V · ∇_V L(V, λ)
Dual 업데이트:   λ_i ← max(0, λ_i + α_λ · g_i(V))
```

여기서:
- `g_i(V)`: i번째 제약 함수 (예: 장애물 침투 깊이, 속도 한계 초과)
- `λ_i`: 라그랑주 승수 (제약 위반 정도에 비례하여 증가)
- `α_V, α_λ`: primal/dual 학습률

제약 위반 기울기 `∇_V g_i`는 SPSA로 근사하거나, 해석적으로 계산한다.

**DBSCAN 클러스터링**:

```
입력: K개 궤적의 비용 및 종점 {(S_k, x_T^k)}
파라미터: eps (이웃 반경), min_samples (최소 이웃 수)

알고리즘:
  1. 각 궤적 쌍의 거리 계산: d(τ_i, τ_j) = ||τ_i - τ_j||
     (궤적 거리 = 제어 시퀀스 L2 거리 또는 상태 궤적 거리)
  2. DBSCAN 클러스터 할당: C_1, C_2, ..., C_P, Noise
  3. 각 클러스터의 대표 선택: τ_p* = argmin_{τ ∈ C_p} S(τ)
  4. 최적 클러스터 선택: p* = argmin_p S(τ_p*)
  5. 최종 출력: U = V_{p*} (최적 클러스터의 최저 비용 궤적)
```

**가중 평균 대신 대표 선택의 이점**:

```
가중 평균: U = Σ ω_k V_k     → 제약 만족 미보장 (볼록 결합)
대표 선택: U = V_{best}       → 원본 샘플이 제약 만족 → 출력도 만족
```
...
```
매 스텝:
  [Phase 1: Constrained Sampling]
  1. 표준 MPPI 노이즈 샘플링: ε_k ~ N(0, σ²I)
  2. V_k = U_nom + ε_k
  3. Primal-Dual 투영 (선택적, n_dual_iters 반복):
     for iter = 1 to n_dual_iters:
       a. 각 V_k의 제약 위반 계산: g_i(V_k)
       b. Primal: V_k ← V_k - α_V · [∇S_k + Σ λ_i · ∇g_i]
       c. Dual:   λ_i ← max(0, λ_i + α_λ · g_i(V_k))

  [Phase 2: Rollout & Cost]
  4. 투영된 V_k로 rollout → 비용 S_k + 제약 페널티

  [Phase 3: Clustering]
  5. 제약 위반 없는 샘플만 필터링 (feasible set F)
  6. |F| < min_feasible이면 전체 샘플 사용 (폴백)
  7. DBSCAN(F의 제어 시퀀스, eps, min_samples)
  8. 단일 클러스터만 존재하면: 해당 클러스터 내 가중 평균
  9. 다중 클러스터: 각 클러스터 대표(최저 비용) 중 최적 선택

  [Phase 4: 출력]
  10. U = V_{p*} (대표 궤적) 또는 최적 클러스터 내 가중 평균
```
...
```python
# 컨트롤러
CSCMPPIController(MPPIController)
  |-- compute_control(): 제약 투영 → rollout → 클러스터 → 대표 선택
  |-- _primal_dual_projection(): K개 시퀀스 동시 제약 투영
  |-- _evaluate_constraints(): 장애물/속도/가속도 제약 평가
  |-- _cluster_trajectories(): DBSCAN 클러스터링 + 대표 선택
  |-- _select_representative(): 클러스터별 최저 비용 궤적
  +-- get_cluster_statistics(): 클러스터 수, feasible 비율, λ 상태

# 파라미터
CSCMPPIParams(MPPIParams)
  |-- csc_obstacles: []           # [(x, y, r), ...] 장애물 목록
  |-- n_dual_iters: 3             # primal-dual 반복 횟수
  |-- dual_lr_primal: 0.01       # primal 학습률 α_V
  |-- dual_lr_dual: 0.1          # dual 학습률 α_λ
  |-- constraint_margin: 0.1      # 제약 안전 마진
  |-- cluster_eps: 0.5            # DBSCAN eps (이웃 반경)
  |-- cluster_min_samples: 3      # DBSCAN 최소 이웃 수
  |-- min_feasible_ratio: 0.1     # 최소 feasible 비율 (폴백 임계)
  |-- use_primal_dual: True       # primal-dual 투영 활성화
  +-- use_clustering: True        # 클러스터링 활성화
```

### 31.5 기존 변형 대비

| 특성 | Vanilla MPPI | DBaS-MPPI | pi-MPPI | CSC-MPPI |
|------|-------------|-----------|---------|----------|
| 제약 처리 | 비용 변환 | barrier state | 투영 (jerk) | **primal-dual (범용)** |
| 해 선택 | 가중 평균 | 가중 평균 | 가중 평균 | **클러스터 대표** |
| 평균 비실현성 | 발생 가능 | 발생 가능 | 발생 가능 | **해결 (대표 선택)** |
| 제약 유형 | 소프트 | 배리어 | jerk/snap | **범용 (장애물, 속도 등)** |
| 다중 모달 | 평균으로 붕괴 | 평균으로 붕괴 | 평균으로 붕괴 | **모드별 분리** |
| 추가 의존성 | 없음 | 없음 | scipy (QP) | sklearn (DBSCAN) |

CSC-MPPI의 핵심 혁신은 **"가중 평균의 비실현가능성" 문제를 근본적으로 해결**하는
것이다. 기존 MPPI 변형들은 모두 가중 평균으로 최종 제어를 산출하므로,
다중 모달 환경에서 평균이 모든 모드 사이의 비실현 영역에 위치할 수 있다.
CSC-MPPI는 클러스터링으로 모드를 분리하고, 각 모드의 대표(원본 샘플)를
선택하므로 출력이 항상 실현 가능한 궤적이다.

---

## 32. T-MPPI (Transformer-based Initialization MPPI)

**Reference**: Zinage et al., arXiv:2412.17118, Dec 2024

### 32.1 핵심 아이디어

표준 MPPI는 매 시간 스텝마다 이전 해를 시간 이동(warm start)하여 초기 제어 시퀀스
`U_init`으로 사용한다. 그러나 환경이 급변하거나 새로운 궤적으로 전환할 때,
이전 해가 부적절한 초기화를 제공하여 수렴이 느려질 수 있다.

**T-MPPI**는 **Transformer**를 사용하여 과거 상태/제어 히스토리로부터
근최적 초기 제어 시퀀스를 예측한다:

```
기존 MPPI: U_init = shift(U_prev)  → 이전 해에 의존, 급변 시 부적절
T-MPPI:    U_init = blend * Transformer(history) + (1-blend) * shift(U_prev)
           → 학습된 초기화 + 기존 warm start 혼합
```

핵심 통찰: MPPI의 **초기화만 개선**하고, 샘플링/가중치 계산은 표준 MPPI를 그대로
사용한다. 따라서 수렴 보장 등 MPPI의 이론적 성질을 유지하면서 수렴 속도를 향상시킨다.
...
```
입력: h_t = (s_{t-L:t}, u_{t-L:t-1})   (L = context_length)
출력: U_pred = Transformer(h_t) ∈ R^{N × nu}

혼합 초기화:
  U_init = α · U_pred + (1-α) · shift(U_prev)
  여기서 α = blend_ratio ∈ [0, 1]

MPPI 최적화:
  U* = MPPI(U_init, K samples)   (표준 MPPI 절차)
```

**온라인 학습**:

```
데이터 수집: (h_t, U*_t) 쌍을 TransformerDataBuffer에 저장
손실 함수:   L(θ) = E[||Transformer_θ(h) - U*||²]   (MSE)
업데이트:    θ ← θ - lr · ∇_θ L(θ)   (Adam, gradient clipping = 1.0)
```
...
```
매 스텝:
  [Phase 1: Transformer 초기화]
  1. 과거 히스토리 구성: h_t = (state_history[-L:], control_history[-L:])
  2. Transformer 순전파: U_pred = Transformer(h_t)
  3. 혼합: U_init = α · U_pred + (1-α) · shift(U_prev)
     (히스토리 부족 시 α=0으로 graceful degradation)

  [Phase 2: 표준 MPPI]
  4. U_init를 중심으로 K개 노이즈 샘플링
  5. Rollout → 비용 계산 → 가중 평균 → U*

  [Phase 3: 데이터 수집 & 온라인 학습]
  6. (h_t, U*) 쌍을 버퍼에 저장
  7. 매 training_interval 스텝마다:
     - 버퍼에서 미니배치 샘플링
     - MSE 손실로 Transformer 가중치 업데이트
```
...
```python
# 컨트롤러
TransformerMPPIController(MPPIController)
  |-- compute_control(): Transformer 초기화 → MPPI → 데이터 수집 → 온라인 학습
  |-- _predict_init(): ControlTransformer 순전파 → U_pred
  |-- _collect_data(): (state_history, control_history, U*) 저장
  |-- _maybe_train(): 주기적 온라인 학습 (MSE + Adam)
  |-- save_model() / _load_model(): 체크포인트 관리
  +-- get_transformer_statistics(): 학습 손실, 예측 사용률

# Transformer 아키텍처
ControlTransformer(nn.Module)
  |-- input_projection: (state_dim + control_dim) → d_model
  |-- PositionalEncoding: sine/cosine PE
  |-- TransformerEncoder: causal self-attention (n_heads, n_layers)
  |-- output_projection: d_model → N × control_dim
  +-- zero-init 출력층: 학습 초기 U_pred ≈ 0 (graceful degradation)

# 데이터 관리
TransformerDataBuffer: Ring buffer (deque) + sample(batch_size)
TransformerTrainer: Adam + MSE + gradient clipping (1.0)

# 파라미터
TransformerMPPIParams(MPPIParams)
  |-- use_transformer_init: True    # Transformer 초기화 활성화
  |-- blend_ratio: 0.5              # Transformer/warm-start 혼합 비율
  |-- transformer_hidden_dim: 128   # d_model
  |-- transformer_n_heads: 4        # 어텐션 헤드 수
  |-- transformer_n_layers: 2       # 인코더 레이어 수
  |-- transformer_context_length: 20  # 히스토리 윈도우
  |-- transformer_lr: 1e-3          # 학습률
  |-- transformer_buffer_size: 5000 # 버퍼 최대 크기
  |-- transformer_min_samples: 100  # 학습 시작 최소 샘플
  |-- transformer_batch_size: 32    # 미니배치 크기
  |-- transformer_n_train_steps: 10 # 학습 반복 수 (per interval)
  |-- transformer_training_interval: 10 # 학습 주기 (스텝)
  +-- online_learning: True         # 온라인 학습 활성화
```

### 32.5 기존 변형 대비

| 특성 | Flow-MPPI | Biased-MPPI | DIAL-MPPI | T-MPPI |
|------|----------|-------------|-----------|--------|
| 학습 대상 | 전체 분포 (CFM) | 정책 혼합 가중치 | 반복 어닐링 | **초기화만** |
| 학습 방법 | 조건부 흐름 매칭 | 사전 정의 정책 | 반복 + 감쇄 | **MSE 지도학습** |
| 추론 비용 | 높음 (ODE 적분) | 낮음 (정책 평가) | 중간 (다중 반복) | **낮음 (1회 순전파)** |
| 샘플링 변경 | 전체 대체 | 혼합 분포 | 감쇄 분포 | **변경 없음** |
| MPPI 호환성 | 분포 대체 | 가중 평균 대체 | 반복 대체 | **완전 호환** |
| Graceful degradation | 학습 전 가우시안 | 항시 다중 정책 | 항시 반복 | **zero-init → 표준 MPPI** |

T-MPPI의 핵심 장점은 **MPPI의 샘플링/가중치 계산을 전혀 변경하지 않는다**는 것이다.
Transformer는 초기화만 제공하므로, 학습이 실패하거나 데이터가 부족해도
표준 MPPI로 자연스럽게 폴백(zero-init → shift warm start)한다.

---

## 33. F-MPPI (Feedback Reuse MPPI)

**Reference**: Belvedere et al., IEEE RA-L 2026, arXiv:2506.14855

### 33.1 핵심 아이디어

표준 MPPI는 매 제어 스텝마다 K개 샘플 rollout → 비용 계산 → 가중 평균의
전체 최적화를 수행한다. 고주파 제어(50Hz+)에서는 이 계산이 병목이 된다.

**F-MPPI**는 **1회 MPPI 풀 솔브**의 결과로부터 **Riccati 피드백 게인**을 계산하고,
이후 `reuse_steps`회 동안 피드백만으로 제어를 보정한다:

```
스텝 0: MPPI 풀 솔브 (~3ms) + Jacobian/Riccati 계산
스텝 1-3: 피드백만 적용 (~0.01ms each)
스텝 4: 다시 MPPI 풀 솔브 + Riccati
...

→ reuse_steps=3이면 75% 계산 절감 (4 스텝 중 3 스텝이 피드백)
```
...
```
명목 궤적: x*_0, x*_1, ..., x*_N  (MPPI 최적 제어로 rollout)
명목 제어: u*_0, u*_1, ..., u*_{N-1}

A_t = ∂f/∂x|_{x*_t, u*_t} ≈ [f(x*_t + εe_i, u*_t) - f(x*_t - εe_i, u*_t)] / (2ε)
B_t = ∂f/∂u|_{x*_t, u*_t} ≈ [f(x*_t, u*_t + εe_j) - f(x*_t, u*_t - εe_j)] / (2ε)
```
...
여기서:
  Q = diag(feedback_weight_Q, ...) — 상태 추적 비용
  R = diag(feedback_weight_R, ...) — 제어 비용
```

**피드백 보정**:

```
u(t) = u*_{step} + K_{step} · (x_{actual} - x*_{step})

여기서:
  x*_{step}: 명목 궤적의 해당 시간 상태
  K_{step}: Riccati에서 계산된 피드백 게인 (angle wrap + gain clip 적용)
```
...
매 스텝:
  if reuse_counter == 0:
    [Full Solve 모드]
    1. 표준 MPPI 최적화: U* = MPPI(state, ref)
    2. 명목 궤적 추출: x*_{0:N} = rollout(state, U*)
    3. 야코비안 계산: A_t, B_t (중앙 차분, ε = jacobian_eps)
    4. 후진 Riccati: P_N → P_0, K_{0:N-1}
    5. 게인 클리핑: K_t = clip(K_t, -gain_clip, gain_clip)
    6. reuse_counter = reuse_steps

  else:
    [Feedback Reuse 모드]
    1. step_idx = reuse_steps - reuse_counter
    2. 오차 계산: e = x_actual - x*_{step_idx}  (angle wrap 적용)
    3. 피드백 보정: u = u*_{step_idx} + K_{step_idx} · e
    4. 제어 클리핑: u = clip(u, u_min, u_max)
    5. reuse_counter -= 1
```
...
```python
# 컨트롤러
FeedbackMPPIController(MPPIController)
  |-- compute_control(): full_solve / feedback_reuse 모드 분기
  |-- _full_solve(): MPPI + 야코비안 + Riccati + 게인 저장
  |-- _feedback_step(): 캐시된 게인으로 피드백 보정 (~0.01ms)
  |-- _compute_jacobians(): 중앙 차분 A_t (nx,nx), B_t (nx,nu)
  |-- _solve_riccati(): 후진 재귀 + np.linalg.solve (역행렬 대신)
  |-- _apply_feedback(): angle wrap + 게인 클리핑 + 제어 클리핑
  |-- _extract_nominal_trajectory(): 순전파 rollout
  +-- get_feedback_statistics(): solve/reuse 횟수, 게인 크기, 보정 노름

# 파라미터
FeedbackMPPIParams(MPPIParams)
  |-- use_feedback: True           # 피드백 재사용 활성화
  |-- reuse_steps: 5               # 피드백 재사용 횟수
  |-- jacobian_eps: 1e-5           # 유한 차분 ε
  |-- feedback_weight_Q: 1.0       # 상태 추적 비용 가중치
  |-- feedback_weight_R: 1.0       # 제어 비용 가중치
  |-- feedback_gain_clip: 10.0     # 게인 클리핑 값
  +-- use_warm_start: True         # warm start 유지
```

### 33.5 기존 변형 대비

| 특성 | Tube-MPPI | R-MPPI | GN-MPPI | F-MPPI |
|------|----------|--------|---------|--------|
| 피드백 구조 | 분리 (MPPI→피드백) | 통합 (MPPI+피드백) | 2차 업데이트 | **Riccati 재사용** |
| 매 스텝 계산 | MPPI + 피드백 | MPPI + 외란 | MPPI + GN | **1/N × MPPI** |
| 계산 절감 | 없음 (매 스텝 MPPI) | 없음 (오히려 증가) | 없음 (GN 추가) | **75%+ (reuse_steps 의존)** |
| 외란 모델링 | 가우시안 외란 | 가우시안/적대 외란 | 없음 | **없음 (상태 오차 보정)** |
| 적용 상황 | 항상 외란 | 강한 외란 | 비용 지형 복잡 | **고주파 제어 + 안정 환경** |

F-MPPI의 핵심 혁신은 **계산 효율**에 있다. Tube-MPPI는 매 스텝마다 MPPI를
수행하고 별도 피드백을 추가하는 반면, F-MPPI는 MPPI를 간헐적으로만 수행하고
그 사이를 Riccati 피드백으로 채운다. 환경이 급변하지 않는 한,
Riccati 피드백은 로컬 편차를 효과적으로 보정한다.

---

## 34. C-MPPI (Contingency MPPI)

**Reference**: Jung, Estornell & Everett, L4DC 2025, arXiv:2412.09777

### 34.1 핵심 아이디어

표준 MPPI는 명목 궤적의 **추적 비용만** 최적화하므로, 궤적 중간에
예상치 못한 상황이 발생해도 **안전한 탈출 경로**가 보장되지 않는다.

**C-MPPI**는 명목 궤적의 **체크포인트 상태들**에서 **비상 계획(contingency)**의
실행 가능성을 평가한다:

```
표준 MPPI: min C_nominal(U)
C-MPPI:    min C_nominal(U) + λ · max_t C_contingency(x_t)

C_contingency(x_t) = min(C_braking(x_t), C_inner_mppi(x_t))
→ 각 체크포인트에서: 급정거 또는 비상 MPPI 중 더 나은 탈출 비용
→ 최악의 체크포인트에서도 비상 탈출 가능해야 함
```
...
```
체크포인트: t_1, t_2, ..., t_C  (C = n_checkpoints, 균등 분포)

각 체크포인트 t_c에서:
  1. 급정거 비용: C_brake(x_{t_c}) = cost(rollout(x_{t_c}, 0, H_cont))
  2. 비상 MPPI 비용: C_mppi(x_{t_c}) = min_U' cost(rollout(x_{t_c}, U', H_cont))
     (K_cont개 샘플로 내부 MPPI)

  비상 비용: C_cont(x_{t_c}) = min(C_brake, C_mppi)

총 비상 비용: C_contingency = max_{c=1,...,C} C_cont(x_{t_c})
```

**안전 페널티**:

```
if C_cont(x_{t_c}) > safe_cost_threshold:
  penalty += safety_cost_weight × (C_cont - threshold)

최종 비용: C_total = C_nominal + λ_cont × C_contingency + penalty
```
...
매 스텝:
  [Phase 1: 명목 MPPI]
  1. K개 노이즈 샘플링, rollout → K개 궤적
  2. 명목 비용 C_nominal_k 계산 (k=1,...,K)

  [Phase 2: 비상 평가]
  3. 체크포인트 인덱스 결정: [N/C, 2N/C, ..., N] (균등)
  4. 각 체크포인트에서 K개 샘플의 상태 추출:
     x_{t_c}^k = trajectory_k[t_c]

  5. 급정거 평가 (배치):
     controls = zeros(K, H_cont, nu)
     brake_cost_k = cost(rollout(x_{t_c}^k, 0, H_cont))

  6. 내부 MPPI 평가 (배치):
     for each sample k:
       inner_cost_k = mini_MPPI(x_{t_c}^k, K_cont samples, H_cont)

  7. 비상 비용: cont_k = max over checkpoints min(brake, inner_mppi)

  [Phase 3: 비용 집계]
  8. total_k = C_nominal_k + λ × cont_k + safety_penalty_k
  9. 표준 MPPI 가중 평균으로 U* 계산
```
...
```python
# 컨트롤러
ContingencyMPPIController(MPPIController)
  |-- compute_control(): 명목 MPPI + 비상 평가 + 비용 집계
  |-- _evaluate_contingency_batch(): K개 샘플 × C개 체크포인트 배치 평가
  |-- _batch_rollout_from_states(): K개 병렬 rollout (서로 다른 초기 상태)
  |-- _batch_inner_mppi(): K개 독립 내부 MPPI (K_cont 샘플 각각)
  |-- _get_checkpoint_indices(): 균등 분포 체크포인트 인덱스
  +-- get_contingency_statistics(): 비상 비용 통계 (평균/최대/위반율)

# 파라미터
ContingencyMPPIParams(MPPIParams)
  |-- n_checkpoints: 5            # 비상 평가 체크포인트 수
  |-- contingency_weight: 1.0     # 비상 비용 가중치 λ
  |-- contingency_samples: 512    # 내부 MPPI 샘플 수
  |-- contingency_horizon: 10     # 비상 rollout 호라이즌
  |-- contingency_sigma_scale: 1.0 # 내부 MPPI 노이즈 스케일
  |-- use_braking_contingency: True  # 급정거 비상 활성화
  |-- use_mppi_contingency: True     # 내부 MPPI 비상 활성화
  |-- safe_cost_threshold: 10.0   # 안전 페널티 임계값
  +-- safety_cost_weight: 100.0   # 안전 위반 페널티 가중치
```

### 34.5 기존 변형 대비

| 특성 | CBF-MPPI | DBaS-MPPI | Shield-MPPI | C-MPPI |
|------|---------|-----------|-------------|--------|
| 안전 메커니즘 | 1-step CBF 제약 | barrier state | QP safety filter | **비상 궤적 계획** |
| 시간 범위 | 1 스텝 | 전체 호라이즌 | 1 스텝 | **비상 호라이즌 H_cont** |
| 안전 보증 | CBF 불변 집합 | barrier 감소 | QP 실행 가능 | **비상 탈출 가능성** |
| 계산 비용 | 낮음 (QP) | 중간 (barrier) | 낮음 (QP) | **높음 (중첩 MPPI)** |
| 다중 전략 | 단일 CBF | 단일 barrier | 단일 필터 | **급정거 + MPPI 비상** |

C-MPPI의 핵심 혁신은 **모든 계획 시점에서 비상 탈출이 가능**함을 보장하는 것이다.
CBF나 Shield는 현재 상태에서의 1-step 안전만 보장하지만, C-MPPI는
궤적 중간의 임의 시점에서도 안전한 비상 경로가 존재함을 확인한다.
대신 내부 MPPI 계산으로 인해 계산 비용이 높다.

---

## 35. DualGuard-MPPI (HJ Safety Value MPPI)

**Reference**: Borquez et al., IEEE RA-L 2025, arXiv:2502.01924

### 35.1 핵심 아이디어

장애물 환경에서 MPPI 샘플의 안전성을 보장하기 위해 **안전 가치 함수 V(x)**를
도입한다. Hamilton-Jacobi (HJ) reachability에서 영감을 받아,
**signed distance + TTC 기반** 안전 가치를 정의하고
**이중 가드(dual guard)** 메커니즘으로 안전을 보장한다:

```
V(x) = min_i (||pos - o_i|| - r_i - margin)   (signed distance)
V(x) > 0: 안전    V(x) < 0: 위험 (장애물 내부)

이중 가드:
1. Sample Guard: K개 샘플의 V(x) 기반 비용 조정 (soft/hard/filter)
2. Nominal Guard: 최종 제어의 안전성 검증 + 기울기 보정
```
...
```
V(x) = min_{i=1,...,M} (||pos(x) - o_i|| - r_i - margin)

속도 보정 (TTC):
  V_vel(x) = V(x) - w_v · max(-dot(v, ∇V), 0) · TTC_factor
  → 장애물에 접근하는 속도가 클수록 V(x)를 더 작게 (더 위험하게)
```
...
```
Soft Guard:
  cost_k += penalty · exp(-decay · V(x_{t}^k))
  → V(x)가 작을수록 비용 기하급수적 증가

Hard Guard:
  if V(x) < threshold:
    u += α · ∇V(x)   (안전 방향으로 기울기 추종)
  → 위험 상태에서 제어를 직접 수정

Filter Guard:
  if min_t V(x_t^k) < 0:
    cost_k += large_penalty  (사실상 해당 샘플 제거)
  → 위험 궤적을 하드하게 필터링
```
...
```
safe_fraction = (V > 0인 샘플 수) / K
if safe_fraction < min_safe_fraction:
  σ_eff = σ · noise_boost_factor
  → 안전 샘플 부족 시 탐색 범위 자동 확대
```
...
매 스텝:
  [Phase 1: 샘플링 + Rollout]
  1. 노이즈 샘플링 (적응 σ 적용)
  2. K개 rollout → 궤적 + 기본 비용

  [Phase 2: Sample Guard]
  3. V(x_t^k) 배치 평가 (K × N × M 장애물)
  4. 속도 페널티 추가 (TTC 기반)
  5. 가드 모드에 따라 비용 조정:
     - soft: 기하급수적 페널티 추가
     - hard: 위험 샘플의 제어 수정
  ...
  [Phase 3: 적응]
  6. safe_fraction 계산
  7. 부족 시 noise_boost 활성화 → 다음 스텝 σ 증가

  [Phase 4: Nominal Guard]
  8. 가중 평균으로 U* 계산
  9. U*로 rollout → 명목 궤적의 V(x) 검사
  10. min V(x) < 0이면: u += α · ∇V(x) 보정
```
...
```python
# 컨트롤러
DualGuardMPPIController(MPPIController)
  |-- compute_control(): 샘플링 → sample guard → nominal guard
  |-- _apply_soft_guard(): exp(-decay·V) 페널티 추가
  |-- _apply_hard_guard(): 기울기 기반 제어 보정
  |-- _apply_filter_guard(): 위험 궤적 하드 필터링
  |-- _compute_velocity_penalty(): TTC + 접근 속도 페널티
  |-- _check_and_boost_noise(): safe_fraction 기반 적응 노이즈
  |-- _guard_nominal(): 최종 제어 안전 보정
  |-- update_obstacles(): 동적 장애물 업데이트
  +-- get_guard_statistics(): 안전 비율, V 통계, 노이즈 부스트

# 안전 가치 함수
SafetyValueFunction
  |-- evaluate(states): 배치 V(x) = min_i(dist_i - r_i - margin)
  |-- evaluate_with_velocity(states): TTC 보정 V(x)
  +-- gradient(states): 최근접 장애물 반대 방향 단위 벡터

# 파라미터
DualGuardMPPIParams(MPPIParams)
  |-- obstacles: []                # [(x, y, r), ...] 장애물 목록
  |-- safety_margin: 0.2           # 안전 마진 (m)
  |-- ttc_horizon: 1.0             # TTC 호라이즌 (s)
  |-- safety_mode: "soft"          # "soft" | "hard" | "filter"
  |-- safety_penalty: 100.0        # soft 가드 페널티 계수
  |-- safety_decay: 10.0           # soft 가드 감쇠율
  |-- use_velocity_penalty: True   # 속도 페널티 활성화
  |-- velocity_penalty_weight: 0.5 # 속도 페널티 가중치
  |-- use_sample_guard: True       # 샘플 가드 활성화
  |-- use_nominal_guard: True      # 명목 가드 활성화
  |-- min_safe_fraction: 0.5       # 최소 안전 비율
  +-- noise_boost_factor: 1.2      # 노이즈 부스트 배율
```

### 35.5 기존 변형 대비

| 특성 | CBF-MPPI | DBaS-MPPI | CSC-MPPI | DualGuard-MPPI |
|------|---------|-----------|----------|----------------|
| 안전 함수 | 해석적 CBF h(x) | barrier B(h) | 제약 g(V) | **signed dist + TTC** |
| 가드 수준 | 1단 (CBF 필터) | 1단 (barrier) | 1단 (primal-dual) | **2단 (sample + nominal)** |
| 속도 인식 | 없음 | 없음 | 없음 | **TTC 기반 속도 페널티** |
| 적응 노이즈 | 없음 | 배리어 기반 | 없음 | **safe_fraction 기반** |
| 모드 선택 | CBF-QP | barrier | primal-dual | **soft/hard/filter 3모드** |
| 동적 장애물 | 외부 업데이트 | update_obstacles | 외부 업데이트 | **update_obstacles** |

DualGuard-MPPI의 핵심 혁신은 **이중 가드 메커니즘**이다.
CBF-MPPI는 최종 제어에만 안전 필터를 적용하고, DBaS는 비용 변환만 하지만,
DualGuard는 (1) 샘플 단계에서 위험 궤적을 억제하고
(2) 최종 제어에서 한 번 더 안전을 검증한다.
또한 TTC 기반 속도 페널티는 장애물에 빠르게 접근하는 상황을 사전에 억제한다.

---

## 36. PR-MPPI (Parameter-Robust MPPI)

**Reference**: Vahs et al., 2026, arXiv:2601.02948

### 36.1 핵심 아이디어

실제 로봇은 모델 파라미터(wheelbase, 마찰 계수, 질량 등)가 명목값과 다를 수 있다.
표준 MPPI는 고정된 명목 모델을 사용하므로, 파라미터 불일치 시 성능이 급감한다.

**PR-MPPI**는 **입자 필터(particle filter)**로 파라미터 belief를 유지하고,
**M개 모델 가설**로 동시 rollout하여 **파라미터 불확실성에 강건한 제어**를 산출한다:

```
표준 MPPI: cost_k = rollout(x, u_k, model_nominal)
PR-MPPI:   cost_k = aggregate_i [w_i · rollout(x, u_k, model(θ_i))]

θ_1, ..., θ_M ~ belief(θ)   (입자 필터로 유지)
w_1, ..., w_M               (관측 기반 Bayesian 가중치)

aggregate: weighted_mean | worst_case | cvar
```
...
```
초기화: θ_i ~ N(θ_nominal, σ²),  w_i = 1/M  (i=1,...,M)

관측 업데이트 (매 스텝):
  1. 예측: x_pred_i = f(x_t, u_t, θ_i)
  2. 관측 오차: e_i = ||x_{t+1} - x_pred_i||²
  3. 우도: L_i = exp(-e_i / (2σ_obs²))
  4. 가중치 업데이트: w_i ∝ L_i · w_i^{prior}
  5. 정규화: w_i = w_i / Σ_j w_j

재샘플링 (ESS < threshold × M):
  Systematic resampling + jitter
  ESS = 1 / Σ_i w_i² → 유효 입자 수
```

**비용 집계**:

```
weighted_mean: cost_k = Σ_i w_i · cost(rollout(x, u_k, model(θ_i)))
               → Bayesian 후방 평균 (기대 비용 최소화)

worst_case:    cost_k = max_i cost(rollout(x, u_k, model(θ_i)))
               → 미니맥스 강건 (최악 파라미터에 대비)

cvar:          cost_k = mean(top α% of {w_i · cost_i})
               → 조건부 위험 가치 (중간 수준 강건성)
```
...
```
매 스텝:
  [Phase 1: 멀티 모델 Rollout]
  1. K개 노이즈 샘플링 (1회, 모든 모델 공유)
  2. M개 모델 각각에서 K개 rollout:
     cost_{i,k} = rollout(x, u_k, model(θ_i))  → (M, K) 비용 행렬

  [Phase 2: 비용 집계]
  3. 집계 모드에 따라 (M, K) → (K,):
     - weighted_mean: Σ w_i · cost_{i,k}
     - worst_case: max_i cost_{i,k}
     - cvar: top-α% 평균

  [Phase 3: MPPI 업데이트]
  4. 집계된 비용으로 표준 MPPI 가중 평균 → U*
...
```python
# 컨트롤러
ParameterRobustMPPIController(MPPIController)
  |-- compute_control(): 멀티 모델 rollout → 비용 집계 → MPPI → 파라미터 학습
  |-- _rollout_all_particles(): M개 모델 × K개 샘플 비용 계산
  |-- _aggregate_costs(): weighted_mean/worst_case/cvar 집계
  |-- _update_history(): 상태/제어 히스토리 윈도우 관리
  |-- _update_parameters(): Bayesian 업데이트 + 재샘플링
  +-- get_parameter_statistics(): 가중 평균/최적/std, ESS, 입자 분포

# 입자 필터
ParameterParticleFilter
  |-- update_weights(): 관측 우도 × 사전 → 사후 가중치
  |-- resample_if_needed(): ESS 기반 systematic resampling + jitter
  |-- get_weighted_mean/best/std(): 점 추정치
  +-- ess: 유효 샘플 크기

# 파라메트릭 모델
_ParametricModel(RobotModel)
  |-- forward_dynamics(): 파라미터 값에 따라 동역학 조정
  +-- wheelbase: nominal/actual 비율로 ω 스케일링

# 파라미터
ParameterRobustMPPIParams(MPPIParams)
  |-- param_name: "wheelbase"      # 추적 파라미터 이름
  |-- param_nominal: 0.5           # 명목 파라미터 값
  |-- param_std: 0.15              # 초기 불확실성 표준편차
  |-- param_min: 0.2               # 파라미터 하한
  |-- param_max: 1.2               # 파라미터 상한
  |-- n_particles: 8               # 입자 수
  |-- aggregation_mode: "weighted_mean"  # 집계 모드
  |-- cvar_alpha: 0.5              # CVaR 신뢰 수준
  |-- online_learning: True        # 온라인 학습 활성화
  |-- min_observations: 10         # 학습 시작 최소 관측 수
  |-- observation_window: 50       # 관측 히스토리 윈도우
  |-- use_resampling: True         # 재샘플링 활성화
  +-- resample_threshold: 0.8      # ESS/M 재샘플링 임계값
```

### 36.5 기존 변형 대비

| 특성 | Tube-MPPI | R-MPPI | BNN-MPPI | PR-MPPI |
|------|----------|--------|---------|---------|
| 불확실성 유형 | 상태 외란 | 상태 외란 | 모델 구조 | **모델 파라미터** |
| 불확실성 표현 | 고정 게인 | 가우시안 외란 | 앙상블 분산 | **입자 필터 belief** |
| 적응 학습 | 없음 | 없음 | 오프라인 학습 | **온라인 Bayesian** |
| 비용 집계 | 명목+피드백 | 실제 궤적 비용 | 불확실성 비용 | **M-model 집계** |
| 파라미터 추정 | 없음 | 없음 | 없음 | **입자 필터 추정** |

PR-MPPI의 핵심 혁신은 **파라미터 불확실성을 명시적으로 모델링**하는 것이다.
Tube-MPPI와 R-MPPI는 상태 외란만 다루고, BNN-MPPI는 모델 구조 불확실성을
앙상블로 포착하지만, PR-MPPI는 특정 물리 파라미터의 belief를 입자 필터로
유지하면서 온라인으로 학습한다. wheelbase 60% 불일치 상황에서도
RMSE 35% 개선을 달성한다.
...
│          │
├─ 하드 제약이 있는가?
│  ├─ Yes ─┬─ jerk/snap 하드 바운드?      → pi-MPPI
│  │       ├─ 범용 제약 (장애물+속도)?    → CSC-MPPI
│  │       └─ 평균 비실현가능성 우려?     → CSC-MPPI
│  └─ No ─┐
│          │
├─ 다중 모달 해가 있는가?
│  ├─ Yes ─┬─ 학습 데이터 있음?   → Flow-MPPI
│  │       ├─ 학습 없이 실시간?   → SVG-MPPI (SVGD)
│  │       ├─ 모드별 분리 필요?   → CSC-MPPI
│  │       └─ 최고 다양성?        → SVMPC
│  └─ No ─┐
│          │
├─ 매끄러운 제어가 필요한가?
│  ├─ Yes ─┬─ 하드 jerk 바운드?    → pi-MPPI
│  │       ├─ 주파수 도메인 제어?  → LP-MPPI
│  │       ├─ 메모리 절약?         → Spline-MPPI
│  │       └─ jerk 비용 최소화?    → Smooth MPPI
│  └─ No ─┐
│          │
├─ 밀집 장애물 / 좁은 통로?
...
├─ 안전이 최우선인가?
│  ├─ Yes ─┬─ 비상 탈출 보장?    → C-MPPI
│  │       ├─ 속도 인식 가드?    → DualGuard-MPPI
│  │       └─ CVaR 가중치?       → Risk-Aware MPPI
│  └─ No ─┐
│          │
├─ 좋은 사전 정책이 있는가?
...
│          │
├─ 지역 최적 탈출?
│  ├─ Yes ─┬─ 학습 없이 즉시?   → DRPA-MPPI
│  │       ├─ 도메인 지식 있음?  → Biased-MPPI
│  │       ├─ 등방 감쇠 OK?      → DIAL-MPPI
│  │       └─ 비용 지형 적응?    → CMA-MPPI
│  └─ No ─┐
│          │
├─ 고주파 제어가 필요한가?
│  ├─ Yes → F-MPPI (Riccati 피드백 재사용, 75%+ 계산 절감)
│  └─ No ─┐
│          │
├─ 학습 기반 초기화가 가능한가?
│  ├─ Yes → T-MPPI (Transformer 초기화, 수렴 가속)
│  └─ No ─┐
│          │
├─ 샘플 효율이 중요한가?
│  ├─ Yes ─┬─ 결정론적 재현성?   → dsMPPI
...
| 밀집 장애물 / 좁은 통로 | DBaS / CSC | barrier + 적응 탐색 / 제약 투영 + 클러스터 |
| 장애물 + 안전 | Risk-Aware + Shield | CVaR + CBF 보장 |
| 다중 경로 | Flow / SVG-MPPI / CSC | 다중 모달 샘플링 / SVGD 가이드 / 클러스터 분리 |
| 좋은 사전 정책 보유 | Residual / Biased | 정책 중심 잔차 최적화 / 정책 혼합 |
| 실시간 제약 | Spline / SVG-MPPI | 메모리/계산 효율 |
| 매끄러운 제어 (하드 바운드) | pi-MPPI | QP/clip 투영으로 jerk/snap 하드 보장 |
| 매끄러운 제어 (소프트) | LP / Smooth / Spline | 주파수 도메인 / 연속 제어 보장 |
| 모델 불확실성 | C2U / Uncertainty / BNN | 불확실성 인식 |
| 정밀 추적 | Tsallis (q>1) | 활용 강화 |
| Local minima 빈발 | DRPA-MPPI | 3-phase 자동 탈출 + 학습 불필요 |
| 샘플 수 제한 (임베디드) | dsMPPI | QMC 결정론적 샘플 K=64로 동등 성능 |
| 하드 제약 + 다중 모달 | CSC-MPPI | primal-dual 투영 + DBSCAN 대표 선택 |
| 재현성 필요 (디버깅) | dsMPPI | 결정론적 시퀀스로 완전 재현 가능 |
| 학습 기반 수렴 가속 | T-MPPI | Transformer 초기화로 수렴 속도 향상 |
| 고주파 제어 (50Hz+) | F-MPPI | Riccati 피드백 재사용 (75%+ 계산 절감) |
| 비상 탈출 보장 | C-MPPI | 중첩 MPPI 비상 계획 (모든 체크포인트) |
| 속도 인식 안전 | DualGuard-MPPI | HJ 가치 함수 + TTC + 이중 가드 |
| 파라미터 불일치 환경 | PR-MPPI | 입자 필터 파라미터 belief + 온라인 학습 |
...
7. 하드 제약 + 매끄러움:
   pi-MPPI + LP-MPPI
   → QP 투영 (jerk 하드 바운드) + Butterworth LPF (주파수 도메인)

8. 최소 샘플 + 강건:
   dsMPPI + Tube
   → QMC K=64 + 외란 대응 (임베디드 환경)

9. Local minima + 다중 모달:
   DRPA + CSC
   → 반발 포텐셜 탈출 + 클러스터 대표 선택 (다중 경로)

10. 밀집 환경 + 제약 보장:
    CSC + DBaS
    → primal-dual 투영 + barrier state (이중 안전망)

11. 고주파 + 강건:
    F-MPPI + Tube
    → Riccati 재사용 + 외란 피드백 (50Hz+ 실시간)
...
13. 학습 초기화 + 전역 탐색:
    T-MPPI + DIAL
    → Transformer warm start + 어닐링 전역 탐색
```
...
사용 가능한 계산 시간에 따른 추천:
  ...
  │             │ dsMPPI                 │ 64   │ QMC 최소 K  │
  │             │ DRPA-MPPI              │ 256  │ 정체 시만+α │
  │             │ F-MPPI (reuse)         │ -    │ ~0.01ms     │
  ├─────────────┼────────────────────────┼──────┼─────────────┤
  │ 5~20ms      │ Smooth + Log           │ 1024 │ CPU         │
  │ (50~200Hz)  │ Tsallis + Log          │ 1024 │             │
  │             │ Risk-Aware             │ 1024 │             │
  │             │ pi-MPPI (clip)         │ 1024 │ 하드 jerk   │
  │             │ LP + pi 조합           │ 1024 │ LPF+투영    │
  ├─────────────┼────────────────────────┼──────┼─────────────┤
  │ 20~50ms     │ Tube + Smooth          │ 2048 │ CPU/GPU     │
  │ (20~50Hz)   │ Uncertainty-Aware      │ 2048 │             │
  │             │ BNN-MPPI               │ 2048 │             │
  │             │ SVG-MPPI (SVGD)        │ 2048 │ G=64        │
  │             │ dsMPPI (CEM 3iter)     │ 256  │ QMC+CEM     │
  ├─────────────┼────────────────────────┼──────┼─────────────┤
  │ 50~100ms    │ DIAL (3 iter)          │ 4096 │ GPU 권장    │
  │ (10~20Hz)   │ C2U-MPPI              │ 4096 │             │
  │             │ Flow-MPPI             │ 4096 │             │
  │             │ CSC-MPPI              │ 2048 │ DBSCAN 포함 │
  │             │ pi-MPPI (qp)          │ 2048 │ QP 투영     │
  │             │ T-MPPI                │ 2048 │ Transformer │
  │             │ DualGuard-MPPI        │ 2048 │ V(x) 평가   │
  │             │ PR-MPPI (M=8)        │ 1024 │ M 모델 반복  │
  ├─────────────┼────────────────────────┼──────┼─────────────┤
  │ > 100ms     │ SVMPC                  │ 256  │ K² 주의     │
  │ (< 10Hz)    │ DIAL + SVG-MPPI        │ 8192 │ GPU 필수    │
  │             │ CSC + primal-dual      │ 4096 │ 제약 반복   │
  │             │ C-MPPI (nested)        │ 512  │ 내부 MPPI   │
  └─────────────┴────────────────────────┴──────┴─────────────┘
```
...
### SVG-MPPI / Projection / Deterministic / DRPA / CSC
19. Honda, K. et al. (2024). "SVG-MPPI: Sampling-based Model Predictive Control with Stein Variational Guided Particles for Autonomous Driving." ICRA. arXiv:2309.11040.
20. Andrejev, A. et al. (2025). "Projection-based Model Predictive Path Integral Control." RA-L. arXiv:2504.10962.
21. Walker, N. et al. (2026). "Deterministic Sampling for Model Predictive Path Integral Control." arXiv:2601.03893.
22. Fuke, R. et al. (2025). "Dynamic Repulsive Potential Augmented MPPI for Reactive Navigation in Local Minima." arXiv:2503.20134.
23. arXiv:2506.16386 (2025). "Constrained Sampling Cluster MPPI: Primal-Dual Projection with Trajectory Clustering for Feasible MPPI."

### T-MPPI / F-MPPI / C-MPPI / DualGuard-MPPI / PR-MPPI
24. Zinage, V. et al. (2024). "Transformer-based Model Predictive Path Integral Control." arXiv:2412.17118.
25. Belvedere, T. et al. (2026). "Feedback-based Model Predictive Path Integral Control." IEEE RA-L. arXiv:2506.14855.
