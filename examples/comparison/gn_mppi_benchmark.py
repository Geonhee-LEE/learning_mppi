#!/usr/bin/env python3
"""
GN-MPPI (Gauss-Newton Accelerated MPPI) 벤치마크: 4-Way x 4 시나리오

방법:
  1. Vanilla MPPI    — 단일 가우시안 (기준선)
  2. DIAL-MPPI       — 다중 반복 + 어닐링
  3. CMA-MPPI        — 공분산 적응
  4. GN-MPPI         — 가우스-뉴턴 2차 업데이트

시나리오 4개:
  A. simple          — 기준선 (장애물 없음, circle)
  B. obstacles       — 3개 원형 장애물 (2차 수렴 검증)
  C. convergence     — 대칭 장애물, figure8 (수렴 속도 비교)
  D. dense_obstacles — 8개 밀집 장애물 (GN 곡률 활용)

Usage:
    PYTHONPATH=. python examples/comparison/gn_mppi_benchmark.py --scenario simple
    ...
    PYTHONPATH=. python examples/comparison/gn_mppi_benchmark.py --no-plot
"""
...
def get_scenarios():
    ...
    return {
        ...
        "obstacles": {
            ...
            "initial_state": np.array([3.0, 0.0, np.pi / 2]),
            "duration": 15.0,
            "description": "3 obstacles — 2nd-order convergence verification",
        },
        "convergence": {
            "name": "C. Convergence",
            "obstacles": [
            ...
            "initial_state": np.array([3.0, 0.0, np.pi / 2]),
            "duration": 15.0,
            "description": "Figure-8 with symmetric obstacles — convergence speed",
        },
        "dense_obstacles": {
            ...
            "initial_state": np.array([3.0, 0.0, np.pi / 2]),
            "duration": 15.0,
            "description": "8 dense obstacles — GN curvature exploitation",
        },
    }
...
def _make_controllers(model, scenario):
    ...
    gn = GNMPPIController(
        model, g_params, cost_function=_make_cost(g_params, obstacles)
    )
...
def compute_metrics(history, scenario):
    ...
    for info in history["infos"]:
        if isinstance(info, dict) and "gn_stats" in info:
            gn_used_ratios.append(info["gn_stats"].get("gn_used_ratio", 0.0))
            cost_improvements.append(info["gn_stats"].get("cost_improvement", 0.0))
    ...
    return {
        ...
        "errors": errors,
        "mean_gn_used_ratio": float(np.mean(gn_used_ratios)) if gn_used_ratios else 0.0,
        "mean_cost_improvement": float(np.mean(cost_improvements)) if cost_improvements else 0.0,
    }
...
def run_live(args):
    ...
    print(f"\n{'=' * 60}")
    print(f"  GN-MPPI Live -- {scenario['name']}")
    ...
    # [0,3] GN Used Ratio (GN only)
    ax_gn = axes[0, 3]
    ax_gn.set_xlabel("Time (s)")
    ax_gn.set_ylabel("GN Used Ratio")
    ax_gn.set_title("GN Step Usage (GN-MPPI)")
    ax_gn.grid(True, alpha=0.3)
    gn_used_data = []
    line_gn, = ax_gn.plot([], [], color=COLORS["GN-MPPI"], linewidth=2)
    ...
    ax_rmse.set_xticklabels(["Van", "DIAL", "CMA", "GN"], fontsize=8)
    ...
    ax_safe.set_xticklabels(["Van", "DIAL", "CMA", "GN"], fontsize=8)
    ...
    def update(frame):
        ...
        for name, ctrl in controllers.items():
            ...
            # GN Used Ratio (GN only)
            if name == "GN-MPPI" and "gn_stats" in info:
                gn_used_data.append(info["gn_stats"]["gn_used_ratio"])
        ...
        if gn_used_data:
            line_gn.set_data(times[:len(gn_used_data)], gn_used_data)
            ax_gn.relim()
            ax_gn.autoscale_view()
    ...
    gif_path = f"plots/gn_mppi_live_{scenario_key}.gif"
    print(f"\n  Saving GIF ({num_steps} frames) ...")
    anim.save(gif_path, writer="pillow", fps=20, dpi=100)
    print(f"  GIF saved: {gif_path}")

    try:
        mp4_path = f"plots/gn_mppi_live_{scenario_key}.mp4"
...
def run_benchmark(args):
    ...
    print(f"\n{'=' * 110}")
    print(f"  GN-MPPI Benchmark: 4-Way Gauss-Newton Comparison")
    ...
    header = (
        ...
        f"{'SolveMs':>10} {'GNUsedRatio':>12} {'CostImpr':>10}"
    )
    if has_obstacles:
    ...
    for r in all_results:
        line = (
            ...
            f"{r['mean_gn_used_ratio']:>12.3f} "
            f"{r['mean_cost_improvement']:>10.4f}"
        )
        if has_obstacles:
...
def _plot_results(results, scenario, scenario_key):
    ...
    # (0,3) GN Used Ratio (GN-MPPI only)
    ax = axes[0, 3]
    for r in results:
        ratios = []
        for info in r["infos"]:
            if isinstance(info, dict) and "gn_stats" in info:
                ratios.append(info["gn_stats"]["gn_used_ratio"])
        if ratios:
            t_plot = np.arange(len(ratios)) * dt
            ax.plot(t_plot, ratios, color=r["color"], label=r["short"], linewidth=1.5)
    if not any("gn_stats" in info for r in results for info in r["infos"]
               if isinstance(info, dict)):
        ax.text(0.5, 0.5, "N/A (GN-MPPI only)", transform=ax.transAxes,
                ha="center", va="center", fontsize=10, alpha=0.5)
    ax.set_xlabel("Time (s)")
    ax.set_ylabel("GN Used Ratio")
    ax.set_title("GN Step Usage")
    ax.legend(fontsize=7)
    ax.grid(True, alpha=0.3)
    ...
    for r in results:
        ...
        if r["mean_gn_used_ratio"] > 0:
            line_txt += f"  GN={r['mean_gn_used_ratio']:.1%}"
    ...
    plt.suptitle(
        f"GN-MPPI Benchmark [{scenario_key}]: "
        f"Vanilla vs DIAL vs CMA vs GN-MPPI",
        fontsize=13, fontweight="bold",
    )
    plt.tight_layout()

    os.makedirs("plots", exist_ok=True)
    out_path = f"plots/gn_mppi_benchmark_{scenario_key}.png"
...
def main():
    parser = argparse.ArgumentParser(description="GN-MPPI Benchmark")
    parser.add_argument(
        "--scenario", default="simple",
        choices=["simple", "obstacles", "convergence", "dense_obstacles"],
    )
