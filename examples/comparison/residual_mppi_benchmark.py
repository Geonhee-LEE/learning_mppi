#!/usr/bin/env python3
"""
Residual-MPPI (사전 정책 + 잔차 최적화) 벤치마크: 4-Way x 4 시나리오

방법:
  1. Vanilla MPPI    — 단일 가우시안 (기준선)
  2. DIAL-MPPI       — 다중 반복 + 어닐링
  3. Biased-MPPI     — 혼합 분포 (정책 + 가우시안)
  4. Residual-MPPI   — 사전 정책 + 잔차 최적화

시나리오 4개:
  A. simple          — 기준선 (장애물 없음, circle)
  B. obstacles       — 3개 원형 장애물 (정책 유도 회피)
  C. policy_quality  — 좋은 정책 vs 나쁜 정책 비교 (figure8)
  D. dense_obstacles — 8개 밀집 장애물 (정책+잔차 협력)

Usage:
    PYTHONPATH=. python examples/comparison/residual_mppi_benchmark.py --scenario simple
    ...
    PYTHONPATH=. python examples/comparison/residual_mppi_benchmark.py --no-plot
"""
...
from mppi_controller.controllers.mppi.ancillary_policies import PurePursuitPolicy
...
def get_scenarios():
    ...
    return {
        ...
        "policy_quality": {
            ...
            "initial_state": np.array([3.0, 0.0, np.pi / 2]),
            "duration": 15.0,
            "description": "Figure-8 with obstacles — good policy vs residual",
        },
        "dense_obstacles": {
            ...
            "initial_state": np.array([3.0, 0.0, np.pi / 2]),
            "duration": 15.0,
            "description": "8 dense obstacles — policy+residual cooperation",
        },
    }
...
def _make_controllers(model, scenario):
    ...
    # 4. Residual-MPPI
    # 장애물이 있으면 KL=0, use_policy_nominal=False (안전 우선, Vanilla 모드로 폴백)
    # 장애물 없으면 정책 중심 + KL 페널티 (정책 활용 극대화)
    if obstacles:
        r_params = ResidualMPPIParams(
            **common,
    ...
    residual = ResidualMPPIController(
        model, r_params,
        cost_function=_make_cost(r_params, obstacles),
        base_policy=PurePursuitPolicy(lookahead=0.5, v_gain=1.0),
    )
...
def compute_metrics(history, scenario):
    ...
    # ResidualNorm (Residual-MPPI 전용)
    residual_norms = []
    for info in history["infos"]:
        if isinstance(info, dict) and "residual_stats" in info:
            residual_norms.append(info["residual_stats"].get("residual_norm", 0.0))
...
def run_live(args):
    ...
    print(f"\n{'=' * 60}")
    print(f"  Residual-MPPI Live -- {scenario['name']}")
    ...
    fig.suptitle(
        f"Residual-MPPI Live -- {scenario['name']}",
    ...
    # [0,3] Residual Norm (Residual only)
    ax_res = axes[0, 3]
    ax_res.set_xlabel("Time (s)")
    ax_res.set_ylabel("Residual Norm")
    ax_res.set_title("Residual Norm (Residual-MPPI)")
    ax_res.grid(True, alpha=0.3)
    residual_norm_data = []
    line_res, = ax_res.plot([], [], color=COLORS["Residual-MPPI"], linewidth=2)
    ...
    ax_rmse.set_xticklabels(["Van", "DIAL", "Biased", "Resid"], fontsize=8)
    ...
    ax_safe.set_xticklabels(["Van", "DIAL", "Biased", "Resid"], fontsize=8)
    ...
    def update(frame):
        ...
        for name, ctrl in controllers.items():
            ...
            # Residual Norm (Residual only)
            if name == "Residual-MPPI" and "residual_stats" in info:
                residual_norm_data.append(info["residual_stats"]["residual_norm"])
        ...
        if residual_norm_data:
            line_res.set_data(times[:len(residual_norm_data)], residual_norm_data)
            ax_res.relim()
    ...
    gif_path = f"plots/residual_mppi_live_{scenario_key}.gif"
    print(f"\n  Saving GIF ({num_steps} frames) ...")
    anim.save(gif_path, writer="pillow", fps=20, dpi=100)
    print(f"  GIF saved: {gif_path}")

    try:
        mp4_path = f"plots/residual_mppi_live_{scenario_key}.mp4"
...
def run_benchmark(args):
    ...
    print(f"\n{'=' * 100}")
    print(f"  Residual-MPPI Benchmark: 4-Way Policy + Residual Comparison")
...
def _plot_results(results, scenario, scenario_key):
    ...
    for r in results:
        ...
        if norms:
            t_plot = np.arange(len(norms)) * dt
            ax.plot(t_plot, norms, color=r["color"], label=r["short"], linewidth=1.5)
    if not any("residual_stats" in info for r in results for info in r["infos"]
               if isinstance(info, dict)):
        ax.text(0.5, 0.5, "N/A (Residual-MPPI only)", transform=ax.transAxes,
                ha="center", va="center", fontsize=10, alpha=0.5)
    ax.set_xlabel("Time (s)")
    ax.set_ylabel("Residual Norm")
    ax.set_title("Residual Norm")
    ax.legend(fontsize=7)
    ax.grid(True, alpha=0.3)
    ...
    for r in results:
        ...
        if r["mean_residual_norm"] > 0:
            line_txt += f"  ResNorm={r['mean_residual_norm']:.3f}"
    ...
    plt.suptitle(
        f"Residual-MPPI Benchmark [{scenario_key}]: "
        f"Vanilla vs DIAL vs Biased vs Residual-MPPI",
        fontsize=13, fontweight="bold",
    )
    plt.tight_layout()

    os.makedirs("plots", exist_ok=True)
    out_path = f"plots/residual_mppi_benchmark_{scenario_key}.png"
...
def main():
    parser = argparse.ArgumentParser(description="Residual-MPPI Benchmark")
    parser.add_argument(
        "--scenario", default="simple",
        choices=["simple", "obstacles", "policy_quality", "dense_obstacles"],
    )
