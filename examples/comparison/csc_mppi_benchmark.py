#!/usr/bin/env python3
"""
CSC-MPPI (Constrained Sampling Cluster MPPI) 벤치마크: 4-Way x 4 시나리오

방법:
  1. Vanilla MPPI    — 단일 가우시안 (기준선)
  2. DBaS-MPPI       — Barrier state + 적응적 탐색 노이즈
  3. Biased-MPPI     — 혼합 분포 (정책 + 가우시안)
  4. CSC-MPPI        — Primal-dual 투영 + DBSCAN 클러스터 선택

시나리오 4개:
  A. simple          — 기준선 (장애물 없음, circle)
  B. obstacles       — 3개 원형 장애물
  C. narrow_passage  — 좁은 통로 (2개 근접 장애물 사이)
  D. dense_obstacles — 8개 밀집 장애물

Usage:
    PYTHONPATH=. python examples/comparison/csc_mppi_benchmark.py --scenario simple
    ...
    PYTHONPATH=. python examples/comparison/csc_mppi_benchmark.py --no-plot
"""
...
def get_scenarios():
    ...
    return {
        ...
        "obstacles": {
            ...
            "initial_state": np.array([3.0, 0.0, np.pi / 2]),
            "duration": 15.0,
            "description": "3 obstacles — constraint projection + clustering",
        },
        "narrow_passage": {
            ...
            "initial_state": np.array([3.0, 0.0, np.pi / 2]),
            "duration": 15.0,
            "description": "Narrow gap (2 obstacles) — feasibility guarantee",
        },
        "dense_obstacles": {
            ...
            "initial_state": np.array([3.0, 0.0, np.pi / 2]),
            "duration": 15.0,
            "description": "8 dense obstacles — cluster selection advantage",
        },
    }
...
def _make_controllers(model, scenario):
    ...
    csc_params = CSCMPPIParams(
        **common,
        ...
        use_projection=len(obstacles) > 0,
        use_clustering=True,
        fallback_to_mppi=True,
    )
    csc = CSCMPPIController(
        model, csc_params, cost_function=_make_cost(csc_params, obstacles)
    )
...
def compute_metrics(history, scenario):
    ...
    for info in history["infos"]:
        if isinstance(info, dict) and "csc_stats" in info:
            stats_list = [i.get("csc_stats", {}) for i in history["infos"]
                          if isinstance(i, dict) and "csc_stats" in i]
            if stats_list:
                mean_clusters = float(np.mean([s.get("n_clusters", 0) for s in stats_list]))
                mean_violation_reduction = float(np.mean(
                    [s.get("violation_reduction", 0) for s in stats_list]
                ))
            break
...
def run_live(args):
    ...
    print(f"\n{'=' * 60}")
    print(f"  CSC-MPPI Live -- {scenario['name']}")
    ...
    # [0,3] Clusters (CSC only)
    ax_clust = axes[0, 3]
    ax_clust.set_xlabel("Time (s)")
    ax_clust.set_ylabel("Clusters")
    ax_clust.set_title("Clusters (CSC-MPPI)")
    ax_clust.grid(True, alpha=0.3)
    cluster_data = []
    line_clust, = ax_clust.plot([], [], color=COLORS["CSC-MPPI"], linewidth=2)
    ...
    ax_rmse.set_xticklabels(["Van", "DBaS", "Biased", "CSC"], fontsize=8)
    ...
    ax_safe.set_xticklabels(["Van", "DBaS", "Biased", "CSC"], fontsize=8)
    ...
    def update(frame):
        ...
        for name, ctrl in controllers.items():
            ...
            # Clusters (CSC only)
            if name == "CSC-MPPI" and "csc_stats" in info:
                cluster_data.append(info["csc_stats"]["n_clusters"])
        ...
        if cluster_data:
            line_clust.set_data(times[:len(cluster_data)], cluster_data)
            ax_clust.relim()
            ax_clust.autoscale_view()
    ...
    gif_path = f"plots/csc_mppi_live_{scenario_key}.gif"
    print(f"\n  Saving GIF ({num_steps} frames) ...")
    anim.save(gif_path, writer="pillow", fps=20, dpi=100)
    print(f"  GIF saved: {gif_path}")

    try:
        mp4_path = f"plots/csc_mppi_live_{scenario_key}.mp4"
...
def run_benchmark(args):
    ...
    print(f"\n{'=' * 100}")
    print(f"  CSC-MPPI Benchmark: 4-Way Constrained Sampling Cluster Comparison")
...
def _plot_results(results, scenario, scenario_key):
    ...
    # (0,3) Clusters / Violation Reduction
    ax = axes[0, 3]
    for r in results:
        clusters = []
        for info in r["infos"]:
            if isinstance(info, dict) and "csc_stats" in info:
                clusters.append(info["csc_stats"]["n_clusters"])
        if clusters:
            t_plot = np.arange(len(clusters)) * dt
            ax.plot(t_plot, clusters, color=r["color"], label=r["short"], linewidth=1.5)
    ax.set_xlabel("Time (s)")
    ax.set_ylabel("Clusters")
    ax.set_title("DBSCAN Clusters (CSC)")
    ax.legend(fontsize=7)
    ax.grid(True, alpha=0.3)
    ...
    for r in results:
        ...
        if r["mean_clusters"] > 0:
            line_txt += f"  Clust={r['mean_clusters']:.1f}"
    ...
    plt.suptitle(
        f"CSC-MPPI Benchmark [{scenario_key}]: "
        f"Vanilla vs DBaS vs Biased vs CSC-MPPI",
        fontsize=13, fontweight="bold",
    )
    plt.tight_layout()

    os.makedirs("plots", exist_ok=True)
    out_path = f"plots/csc_mppi_benchmark_{scenario_key}.png"
...
def main():
    parser = argparse.ArgumentParser(description="CSC-MPPI Benchmark")
    parser.add_argument(
        "--scenario", default="simple",
        choices=["simple", "obstacles", "narrow_passage", "dense_obstacles"],
    )
