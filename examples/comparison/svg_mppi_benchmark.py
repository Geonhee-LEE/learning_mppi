#!/usr/bin/env python3
"""
SVG-MPPI (Stein Variational Guided MPPI) 벤치마크: 4-Way x 4 시나리오

Honda et al., "Stein Variational Guided MPPI", ICRA 2024, arXiv:2309.11040

방법:
  1. Vanilla MPPI    — 단일 가우시안 (기준선)
  2. DIAL-MPPI       — 다중 반복 + 어닐링
  3. CMA-MPPI        — 공분산 적응
  4. SVG-MPPI        — Stein Variational Guided 파티클 최적화

시나리오 4개:
  A. simple          — 기준선 (장애물 없음, circle)
  B. obstacles       — 3개 원형 장애물 (SVGD 모드 탐색)
  C. multimodal      — 대칭 장애물 (2경로), figure8 (다중 모드 비용)
  D. dense_obstacles — 8개 밀집 장애물 (커널 반발력 효과)

Usage:
    PYTHONPATH=. python examples/comparison/svg_mppi_benchmark.py --scenario simple
    ...
    PYTHONPATH=. python examples/comparison/svg_mppi_benchmark.py --no-plot
"""
...
def get_scenarios():
    ...
    return {
        ...
        "obstacles": {
            ...
            "initial_state": np.array([3.0, 0.0, np.pi / 2]),
            "duration": 15.0,
            "description": "3 obstacles — SVGD mode exploration test",
        },
        "multimodal": {
            "name": "C. Multimodal",
            "obstacles": [
            ...
            "initial_state": np.array([3.0, 0.0, np.pi / 2]),
            "duration": 15.0,
            "description": "Symmetric obstacles + figure-8 — multimodal cost landscape",
        },
        "dense_obstacles": {
            ...
            "initial_state": np.array([3.0, 0.0, np.pi / 2]),
            "duration": 15.0,
            "description": "8 dense obstacles — kernel repulsion diversity",
        },
    }
...
def _make_controllers(model, scenario):
    ...
    svg = SVGMPPIController(
        model, s_params, cost_function=_make_cost(s_params, obstacles)
    )
...
def compute_metrics(history, scenario):
    ...
    # SVG stats (SVG-MPPI 전용)
    guide_improvements = []
    for info in history["infos"]:
        if isinstance(info, dict) and "svg_stats" in info:
            guide_improvements.append(
                info["svg_stats"].get("guide_cost_improvement", 0.0)
            )
    ...
    return {
        ...
        "errors": errors,
        "mean_guide_improvement": (
            float(np.mean(guide_improvements)) if guide_improvements else 0.0
        ),
    }
...
def run_live(args):
    ...
    print(f"\n{'=' * 60}")
    print(f"  SVG-MPPI Live -- {scenario['name']}")
    ...
    # [0,3] Guide Cost Improvement (SVG only)
    ax_svg = axes[0, 3]
    ax_svg.set_xlabel("Time (s)")
    ax_svg.set_ylabel("Cost Improvement")
    ax_svg.set_title("SVGD Guide Improvement (SVG-MPPI)")
    ax_svg.grid(True, alpha=0.3)
    svg_improvement_data = []
    line_svg, = ax_svg.plot([], [], color=COLORS["SVG-MPPI"], linewidth=2)
    ...
    ax_rmse.set_xticklabels(["Van", "DIAL", "CMA", "SVG"], fontsize=8)
    ...
    ax_safe.set_xticklabels(["Van", "DIAL", "CMA", "SVG"], fontsize=8)
    ...
    def update(frame):
        ...
        for name, ctrl in controllers.items():
            ...
            # SVG Guide Improvement (SVG only)
            if name == "SVG-MPPI" and "svg_stats" in info:
                svg_improvement_data.append(
                    info["svg_stats"]["guide_cost_improvement"]
                )
        ...
        if svg_improvement_data:
            line_svg.set_data(times[:len(svg_improvement_data)], svg_improvement_data)
            ax_svg.relim()
            ax_svg.autoscale_view()
    ...
    gif_path = f"plots/svg_mppi_live_{scenario_key}.gif"
    print(f"\n  Saving GIF ({num_steps} frames) ...")
    anim.save(gif_path, writer="pillow", fps=20, dpi=100)
    print(f"  GIF saved: {gif_path}")

    try:
        mp4_path = f"plots/svg_mppi_live_{scenario_key}.mp4"
...
def run_benchmark(args):
    ...
    print(f"\n{'=' * 110}")
    print(f"  SVG-MPPI Benchmark: 4-Way Stein Variational Guided Comparison")
    ...
    for r in all_results:
        line = (
            ...
            f"{r['mean_guide_improvement']:>12.4f}"
        )
        if has_obstacles:
...
def _plot_results(results, scenario, scenario_key):
    ...
    # (0,3) Guide Improvement (SVG only)
    ax = axes[0, 3]
    for r in results:
        improvements = []
        for info in r["infos"]:
        ...
        if improvements:
            t_plot = np.arange(len(improvements)) * dt
            ax.plot(t_plot, improvements, color=r["color"],
                    label=r["short"], linewidth=1.5)
    if not any("svg_stats" in info for r in results for info in r["infos"]
               if isinstance(info, dict)):
        ax.text(0.5, 0.5, "N/A (SVG-MPPI only)", transform=ax.transAxes,
                ha="center", va="center", fontsize=10, alpha=0.5)
    ax.set_xlabel("Time (s)")
    ax.set_ylabel("Cost Improvement")
    ax.set_title("SVGD Guide Improvement")
    ax.legend(fontsize=7)
    ax.grid(True, alpha=0.3)
    ...
    for r in results:
        ...
        if r["mean_guide_improvement"] != 0.0:
            line_txt += f"  GI={r['mean_guide_improvement']:.2f}"
    ...
    plt.suptitle(
        f"SVG-MPPI Benchmark [{scenario_key}]: "
        f"Vanilla vs DIAL vs CMA vs SVG-MPPI",
        fontsize=13, fontweight="bold",
    )
    plt.tight_layout()

    os.makedirs("plots", exist_ok=True)
    out_path = f"plots/svg_mppi_benchmark_{scenario_key}.png"
...
def main():
    parser = argparse.ArgumentParser(description="SVG-MPPI Benchmark")
    parser.add_argument(
        "--scenario", default="simple",
        choices=["simple", "obstacles", "multimodal", "dense_obstacles"],
    )
