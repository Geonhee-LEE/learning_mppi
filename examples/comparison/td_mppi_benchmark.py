#!/usr/bin/env python3
"""
TD-MPPI (Temporal-Difference MPPI) 벤치마크: 4-Way x 4 시나리오

방법:
  1. Vanilla MPPI (N=30)  — 긴 호라이즌 기준선
  2. Vanilla MPPI (N=10)  — 짧은 호라이즌 (성능 저하 예상)
  3. DIAL-MPPI (N=10)     — 반복 + 어닐링 (짧은 호라이즌 보상 시도)
  4. TD-MPPI (N=10)       — TD value function으로 호라이즌 보상

시나리오 4개:
  A. simple          — 기준선 (장애물 없음, circle)
  B. obstacles       — 3개 원형 장애물
  C. short_horizon   — N=10 (TD advantage 핵심 검증)
  D. long_episode    — 40s (학습 시간 충분 → TD 성능 향상)

Usage:
    PYTHONPATH=. python examples/comparison/td_mppi_benchmark.py --scenario simple
    ...
    PYTHONPATH=. python examples/comparison/td_mppi_benchmark.py --no-plot
"""
...
def get_scenarios():
    ...
    return {
        ...
        "obstacles": {
            ...
            "initial_state": np.array([3.0, 0.0, np.pi / 2]),
            "duration": 15.0,
            "description": "3 obstacles — safety with short horizon",
        },
        "short_horizon": {
            "name": "C. Short Horizon (TD Advantage)",
            "obstacles": [],
            "trajectory_fn": lambda t: figure_eight_trajectory(t, scale=2.5),
            "initial_state": np.array([2.5, 0.0, np.pi / 2]),
            "duration": 20.0,
            "description": "N=10 figure8 — TD-MPPI advantage over Vanilla(N=10)",
        },
        "long_episode": {
            "name": "D. Long Episode (Learning Improves)",
            "obstacles": [
                (2.5, 1.0, 0.4),
                (-1.5, 2.5, 0.4),
                (-2.0, -1.5, 0.4),
            ...
            "initial_state": np.array([3.0, 0.0, np.pi / 2]),
            "duration": 40.0,
            "description": "40s episode — TD learning improves over time",
        },
    }
...
def _make_controllers(model, scenario):
    ...
    # 1. Vanilla (N=30) — 기준선
    v30_params = MPPIParams(**COMMON_BASE, N=30)
    vanilla_30 = MPPIController(
        model, v30_params, cost_function=_make_cost(v30_params, obstacles)
    )

    # 2. Vanilla (N=10) — 짧은 호라이즌
    v10_params = MPPIParams(**COMMON_BASE, N=10)
    vanilla_10 = MPPIController(
        model, v10_params, cost_function=_make_cost(v10_params, obstacles)
    )

    # 3. DIAL-MPPI (N=10)
    d_params = DIALMPPIParams(
        **COMMON_BASE, N=10,
        n_diffuse_init=8, n_diffuse=3,
    ...
    td_mppi = TDMPPIController(
        model, td_params, cost_function=_make_cost(td_params, obstacles)
    )

    return {
        "Vanilla (N=30)": vanilla_30,
        "Vanilla (N=10)": vanilla_10,
        "DIAL (N=10)": dial,
        "TD-MPPI (N=10)": td_mppi,
    }
...
def compute_metrics(history, scenario):
    ...
    for info in history["infos"]:
        if isinstance(info, dict) and "td_stats" in info:
            td_stats = info["td_stats"]
            if td_stats["td_loss"] > 0:
                td_loss_list.append(td_stats["td_loss"])
            if td_stats["terminal_value_mean"] != 0.0:
                terminal_val_list.append(td_stats["terminal_value_mean"])
    ...
    return {
        ...
        "errors": errors,
        "td_loss_list": td_loss_list,
        "terminal_val_list": terminal_val_list,
    }
...
def run_live(args):
    ...
    print(f"\n{'=' * 60}")
    print(f"  TD-MPPI Live — {scenario['name']}")
    ...
    ax_td.set_ylabel("TD Loss")
    ax_td.set_title("TD Learning Loss")
    ax_td.grid(True, alpha=0.3)
    td_loss_data = []
    td_times_data = []
    line_td, = ax_td.plot([], [], color=COLORS["TD-MPPI (N=10)"], linewidth=2)
    ...
    ax_rmse.set_xticklabels(["V30", "V10", "DIAL", "TD"], fontsize=8)

    # [1,2] Clearance / ESS bars
    ax_safe = axes[1, 2]
    ...
    if has_obstacles:
        ...
        ax_safe.set_ylabel("Mean ESS")
        ax_safe.set_title("Sampling Efficiency")
    ...
    ax_safe.set_xticklabels(["V30", "V10", "DIAL", "TD"], fontsize=8)
    ...
    def update(frame):
        ...
        for name, ctrl in controllers.items():
            ...
            # TD loss
            if name == "TD-MPPI (N=10)" and "td_stats" in info:
                td_loss = info["td_stats"]["td_loss"]
                if td_loss > 0:
                    td_loss_data.append(td_loss)
                    td_times_data.append(t)

        sim_t[0] += dt

        times_first = np.array(data[bar_names[0]]["times"])
        ...
        for name in controllers:
            ...
            if len(xy) > 0:
                ...
                t_arr = np.array(data[name]["times"])
                lines_err[name].set_data(t_arr, data[name]["errors"])
                lines_ess[name].set_data(t_arr, data[name]["ess"])
                ctrl_arr = np.array(data[name]["controls"])
                if len(ctrl_arr) > 0:
                    lines_ctrl[name].set_data(t_arr[:len(ctrl_arr)], ctrl_arr[:, 0])

        if td_loss_data:
            line_td.set_data(td_times_data, td_loss_data)
            ax_td.relim()
        ...
        if has_obstacles:
            ...
            for i, name in enumerate(bar_names):
                ...
                bars_safe[i].set_height(mean_ess)
            all_ess = [float(np.mean(data[n]["ess"])) for n in bar_names
                       if data[n]["ess"]]
            if all_ess:
                ax_safe.set_ylim(0, max(all_ess) * 1.3 + 1)
        ...
        for name in controllers:
            ...
            short = name.replace("Vanilla ", "V").replace("DIAL ", "DIAL").replace("TD-MPPI ", "TD")
            lines.append(f"{short:>14}: RMSE={rmse:.3f} ESS={ess:.0f}")
        info_text.set_text("\n".join(lines))
    ...
    gif_path = f"plots/td_mppi_live_{scenario_key}.gif"
    print(f"\n  Saving GIF ({num_steps} frames) ...")
    anim.save(gif_path, writer="pillow", fps=20, dpi=100)
    print(f"  GIF saved: {gif_path}")

    try:
        mp4_path = f"plots/td_mppi_live_{scenario_key}.mp4"
...
def run_benchmark(args):
    ...
    print(f"\n{'=' * 100}")
    print(f"  TD-MPPI Benchmark: Temporal-Difference Value + Short Horizon")
    ...
    for r in all_results:
        if r["td_loss_list"]:
            print(f"\n  TD Learning: {len(r['td_loss_list'])} updates, "
                  f"final loss={r['td_loss_list'][-1]:.6f}")
            if r["terminal_val_list"]:
                print(f"  Terminal Value: mean={np.mean(r['terminal_val_list']):.4f}, "
                      f"final={r['terminal_val_list'][-1]:.4f}")
...
def _plot_results(results, scenario, scenario_key):
    ...
    # (0,3) TD Loss (TD-MPPI only)
    ax = axes[0, 3]
    for r in results:
        if r["td_loss_list"]:
            t_losses = np.linspace(0, duration, len(r["td_loss_list"]))
            ax.plot(t_losses, r["td_loss_list"], color=r["color"],
                    label=r["short"], linewidth=1.5)
    ax.set_xlabel("Time (s)")
    ax.set_ylabel("TD Loss")
    ax.set_title("TD Learning Loss")
    ax.legend(fontsize=7)
    ax.grid(True, alpha=0.3)
    ...
    # (1,2) Collision/Clearance or ESS bars
    ax = axes[1, 2]
    if has_obstacles:
    ...
    for r in results:
        line_txt = (
            f"{r['short']:>10}: RMSE={r['rmse']:.4f}  ESS={r['mean_ess']:.0f}  "
    ...
    plt.suptitle(
        f"TD-MPPI Benchmark [{scenario_key}]: "
        f"Vanilla(N=30) vs Vanilla(N=10) vs DIAL(N=10) vs TD-MPPI(N=10)",
        fontsize=13, fontweight="bold",
    )
    plt.tight_layout()

    os.makedirs("plots", exist_ok=True)
    out_path = f"plots/td_mppi_benchmark_{scenario_key}.png"
...
def main():
    parser = argparse.ArgumentParser(description="TD-MPPI Benchmark")
    parser.add_argument(
        "--scenario", default="simple",
        choices=["simple", "obstacles", "short_horizon", "long_episode"],
    )
