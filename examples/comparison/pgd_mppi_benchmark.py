#!/usr/bin/env python3
"""
PGD-MPPI (Preconditioned Gradient Descent MPPI) 벤치마크: 4-Way x 4 시나리오

방법:
  1. Vanilla MPPI         — 표준 단일 스텝 (baseline)
  2. PGD (1-step, α=1)    — 구조적으로 Vanilla 와 동일 (전처리 경사 1스텝)
  3. PGD (multi-step)     — n_grad_steps=4, step_size=0.7 (다중 경사 스텝)
  4. PGD (adaptive-cov)   — n_grad_steps=3, adapt_covariance=True (공분산 전처리)

시나리오 4개:
  A. simple        — 원형 궤적, 장애물 없음 (기준선 비교)
  B. obstacles     — 3개 장애물, 원형 궤적
  C. local_minima  — 큰 장애물 1개 (r=0.8), local minima 탈출
  D. aggressive    — 8자 궤적 (figure8), 민첩성 vs 안정성

Usage:
    PYTHONPATH=. python examples/comparison/pgd_mppi_benchmark.py --scenario simple
    ...
    PYTHONPATH=. python examples/comparison/pgd_mppi_benchmark.py --no-plot
"""
...
COLORS = {
    "Vanilla MPPI": "#2196F3",
    "PGD (1-step)": "#9C27B0",
    "PGD (multi-step)": "#4CAF50",
    "PGD (adapt-cov)": "#E91E63",
}

SHORT = {
    "Vanilla MPPI": "Van",
    "PGD (1-step)": "PGD-1",
    "PGD (multi-step)": "PGD-M",
    "PGD (adapt-cov)": "PGD-C",
}
...
def get_scenarios():
    ...
    return {
        "simple": {
            ...
            "description": "No obstacles, no noise. Baseline tracking comparison.",
        },
        "obstacles": {
        ...
        "local_minima": {
            ...
            "duration": 15.0,
            "process_noise_std": np.array([0.0, 0.0, 0.0]),
            "description": "One large obstacle (r=0.8). Multi-step gradient escape.",
        },
        "aggressive": {
            "name": "D. Aggressive Figure-8",
            "obstacles": [],
            "trajectory_fn": lambda t: figure_eight_trajectory(t, scale=3.0, period=12.0),
            "initial_state": np.array([0.0, 0.0, np.pi / 4]),
            "duration": 14.0,
            "process_noise_std": np.array([0.0, 0.0, 0.0]),
            "description": "Figure-8 trajectory. Agility vs stability.",
        },
    }
...
def _make_controllers(model, scenario):
    ...
    # 2. PGD (1-step, α=1) — 구조적으로 Vanilla 와 동등
    p1_params = PGDMPPIParams(
        **common,
        step_size=1.0,
        n_grad_steps=1,
        adapt_covariance=False,
    )
    pgd1 = PGDMPPIController(
        model, p1_params, cost_function=_make_cost(p1_params, obstacles),
    )

    # 3. PGD (multi-step)
    pm_params = PGDMPPIParams(
        **common,
        step_size=0.7,
        n_grad_steps=4,
    ...
    pgd_multi = PGDMPPIController(
        model, pm_params, cost_function=_make_cost(pm_params, obstacles),
    )

    # 4. PGD (adaptive-cov)
    pc_params = PGDMPPIParams(
        **common,
        step_size=0.8,
        n_grad_steps=3,
    ...
    pgd_cov = PGDMPPIController(
        model, pc_params, cost_function=_make_cost(pc_params, obstacles),
    )

    return {
        "Vanilla MPPI": vanilla,
        "PGD (1-step)": pgd1,
        "PGD (multi-step)": pgd_multi,
        "PGD (adapt-cov)": pgd_cov,
    }
...
def compute_metrics(history, scenario):
    ...
    # PGD stats (mean grad_norm, mean sigma_scale)
    grad_norms = []
    sigma_scales = []
    for info in history["infos"]:
        if isinstance(info, dict) and "pgd_stats" in info:
            grad_norms.append(info["pgd_stats"]["grad_norm"])
            sigma_scales.append(float(np.mean(info["pgd_stats"]["sigma_scale"])))
    ...
    return {
        ...
        "mean_sigma_scale": float(np.mean(sigma_scales)) if sigma_scales else 1.0,
        "errors": errors,
    }
...
def run_live(args):
    ...
    print(f"\n{'=' * 60}")
    print(f"  PGD-MPPI Live -- {scenario['name']}")
    ...
    def update(frame):
        ...
        for name in controllers:
            ...
            lines.append(f"{SHORT[name]:>6}: RMSE={rmse:.3f} ESS={ess:.0f} ms={avg_ms:.1f}")
        info_text.set_text("\n".join(lines))
    ...
    gif_path = f"plots/pgd_mppi_live_{scenario_key}.gif"
    print(f"\n  Saving GIF ({num_steps} frames) ...")
    anim.save(gif_path, writer="pillow", fps=20, dpi=100)
    print(f"  GIF saved: {gif_path}")

    try:
        mp4_path = f"plots/pgd_mppi_live_{scenario_key}.mp4"
...
def run_benchmark(args):
    ...
    print(f"\n{'=' * 80}")
    print(f"  PGD-MPPI Benchmark: 4-Way Comparison")
    ...
    header = (
        ...
        f"{'MeanESS':>10} {'SolveMs':>10} {'GradNorm':>10} {'SigScale':>10}"
    )
    if has_obstacles:
    ...
    for r in all_results:
        line = (
            ...
            f"{r['mean_grad_norm']:>10.4f} "
            f"{r['mean_sigma_scale']:>10.3f}"
        )
        if has_obstacles:
...
def _plot_results(results, scenario, scenario_key):
    ...
    # (1,3) Mean grad norm bar
    ax = axes[1, 3]
    grad_norms = [r["mean_grad_norm"] for r in results]
    bars_gn = ax.bar(names, grad_norms, color=colors, alpha=0.8)
    for bar, val in zip(bars_gn, grad_norms):
        ax.text(bar.get_x() + bar.get_width() / 2, max(bar.get_height(), 0) + 0.001,
                f"{val:.3f}", ha="center", va="bottom", fontsize=9)
    ax.set_ylabel("Mean Grad Norm")
    ax.set_title("PGD Gradient Norm")
    ax.grid(True, alpha=0.3, axis="y")

    plt.suptitle(
        f"PGD-MPPI Benchmark [{scenario_key}]: "
        f"Vanilla vs PGD-1 vs PGD-Multi vs PGD-AdaptCov",
        fontsize=13, fontweight="bold",
    )
    plt.tight_layout()

    os.makedirs("plots", exist_ok=True)
    out_path = f"plots/pgd_mppi_{scenario_key}.png"
...
def main():
    parser = argparse.ArgumentParser(description="PGD-MPPI Benchmark")
    parser.add_argument(
        "--scenario", default="simple",
        choices=["simple", "obstacles", "local_minima", "aggressive"],
    )
