#!/usr/bin/env python3
"""
LP-MPPI (Low-Pass MPPI) 벤치마크: 4-Way x 4 시나리오

방법:
  1. Vanilla MPPI    — 가우시안 노이즈 (기준선)
  2. Colored-Noise   — OU 프로세스 (시간 영역 상관)
  3. Smooth-MPPI     — Input-Lifting + Jerk cost (제어 공간 변환)
  4. LP-MPPI         — Butterworth LPF (주파수 영역 필터링)
...
Usage:
    PYTHONPATH=. python examples/comparison/lp_mppi_benchmark.py --scenario simple
    ...
    PYTHONPATH=. python examples/comparison/lp_mppi_benchmark.py --no-plot
"""
...
from mppi_controller.controllers.mppi.sampling import ColoredNoiseSampler
...
def get_scenarios():
    ...
    return {
        ...
        "obstacles": {
            ...
            "initial_state": np.array([3.0, 0.0, np.pi / 2]),
            "duration": 15.0,
            "description": "3 obstacles — smooth avoidance trajectories",
            "sigma_override": None,
        },
        "aggressive": {
        ...
        "actuator_stress": {
            ...
            "initial_state": np.array([3.0, 0.0, np.pi / 2]),
            "duration": 15.0,
            "description": "High noise (sigma=1.0) + 2 obstacles — LP filtering maximized",
...
def _make_controllers(model, scenario):
    ...
    # 2. Colored-Noise (OU)
    c_params = MPPIParams(**common)
    colored_sampler = ColoredNoiseSampler(
        sigma=sigma, theta=np.array([2.0, 2.0]), dt=common["dt"],
    )
    colored = MPPIController(model, c_params, cost_function=_make_cost(c_params, obstacles),
                             noise_sampler=colored_sampler)
...
def compute_metrics(history, scenario):
    ...
    # MSSD + Jerk
    mssd, mean_jerk = _compute_control_smoothness(
...
def run_live(args):
    ...
    print(f"\n{'=' * 60}")
    print(f"  LP-MPPI Live — {scenario['name']}")
    ...
    # [0,3] 주파수 스펙트럼 (FFT, 마지막에 한번)
    ax_fft = axes[0, 3]
    ax_fft.set_xlabel("Frequency (Hz)")
    ax_fft.set_ylabel("Power")
    ax_fft.set_title("Frequency Spectrum (final)")
    ...
    ax_rmse.set_xticklabels(["Van", "Color", "Smooth", "LP"], fontsize=8)
    ...
    ax_mssd_bar.set_xticklabels(["Van", "Color", "Smooth", "LP"], fontsize=8)
    ...
    def update(frame):
        ...
        for name, ctrl in controllers.items():
            ...
            if len(ctrls) >= 3:
                mssd_val, _ = _compute_control_smoothness(ctrls)
        ...
        for name in controllers:
            ...
            short = name.replace(" MPPI", "").replace("-MPPI", "").replace("-Noise", "")
        ...
        if frame == num_steps - 1:
            ...
            for name, color in COLORS.items():
                ...
                if len(ctrls) > 10:
                    fft = np.fft.rfft(ctrls[:, 0])
                    freqs = np.fft.rfftfreq(len(ctrls), d=dt)
                    power = np.abs(fft) ** 2
                    power_db = 10 * np.log10(power + 1e-10)
                    ax_fft.plot(freqs[1:], power_db[1:], color=color,
                                linewidth=1, label=name.replace(" MPPI", ""), alpha=0.7)
            ax_fft.legend(fontsize=6)
    ...
    gif_path = f"plots/lp_mppi_live_{scenario_key}.gif"
    print(f"\n  Saving GIF ({num_steps} frames) ...")
    anim.save(gif_path, writer="pillow", fps=20, dpi=100)
    print(f"  GIF saved: {gif_path}")

    try:
        mp4_path = f"plots/lp_mppi_live_{scenario_key}.mp4"
    ...
    print(f"{'=' * 80}")
    print(f"  {'Method':<18} {'RMSE':>8} {'MaxError':>10} {'MSSD':>10} {'MeanJerk':>10}")
    print(f"  {'-' * 58}")
    for name in controllers:
        errs = data[name]["errors"]
        rmse = np.sqrt(np.mean(np.array(errs) ** 2)) if errs else 0
        ...
        ctrls = data[name]["controls"]
        mssd_val, jerk_val = _compute_control_smoothness(ctrls) if len(ctrls) >= 3 else (0, 0)
        print(f"  {name:<18} {rmse:>8.4f} {me:>10.4f} {mssd_val:>10.4f} {jerk_val:>10.4f}")
    print(f"{'=' * 80}\n")
...
def run_benchmark(args):
    ...
    print(f"\n{'=' * 90}")
    print(f"  LP-MPPI Benchmark: 4-Way Smoothness Comparison")
    ...
    header = (
        ...
        f"{'SolveMs':>10} {'MSSD':>10} {'MeanJerk':>10}"
    )
    if has_obstacles:
...
def _plot_results(results, scenario, scenario_key):
    ...
    for r in results:
        controls = r["controls"]
        if len(controls) > 10:
            fft = np.fft.rfft(controls[:, 0])
            freqs = np.fft.rfftfreq(len(controls), d=dt)
            power = np.abs(fft) ** 2
            power_db = 10 * np.log10(power + 1e-10)
            ax.plot(freqs[1:], power_db[1:], color=r["color"], label=r["short"],
    ...
    for r in results:
        lines.append(
            ...
            f"Jerk={r['mean_jerk']:.4f}  ESS={r['mean_ess']:.0f}  "
    ...
    plt.suptitle(
        f"LP-MPPI Benchmark [{scenario_key}]: "
        f"Vanilla vs Colored-Noise vs Smooth vs LP-MPPI",
        fontsize=13, fontweight="bold",
    )
    plt.tight_layout()

    os.makedirs("plots", exist_ok=True)
    out_path = f"plots/lp_mppi_benchmark_{scenario_key}.png"
...
def main():
    parser = argparse.ArgumentParser(description="LP-MPPI Benchmark")
    parser.add_argument(
