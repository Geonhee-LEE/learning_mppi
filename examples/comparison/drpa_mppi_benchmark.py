#!/usr/bin/env python3
"""
DRPA-MPPI (Dynamic Repulsive Potential Augmented MPPI) 벤치마크: 4-Way x 4 시나리오

방법:
  1. Vanilla MPPI    — 단일 가우시안 (기준선)
  2. DIAL-MPPI       — 다중 반복 + 어닐링
  3. Biased-MPPI     — 혼합 분포 (정책 + 가우시안)
  4. DRPA-MPPI       — 동적 반발 포텐셜 + 정체 감지

시나리오 4개:
  A. simple          — 기준선 (장애물 없음, circle)
  B. obstacles       — 3개 원형 장애물 (반발 포텐셜 효과)
  C. local_minima    — 큰 장애물 (r=0.8, 정체 감지 + 반발 탈출)
  D. u_shape_trap    — U자형 장애물 트랩 (local minima의 극단적 케이스)

Usage:
    PYTHONPATH=. python examples/comparison/drpa_mppi_benchmark.py --scenario simple
    ...
    PYTHONPATH=. python examples/comparison/drpa_mppi_benchmark.py --no-plot
"""
...
def get_scenarios():
    ...
    return {
        ...
        "obstacles": {
            ...
            "initial_state": np.array([3.0, 0.0, np.pi / 2]),
            "duration": 15.0,
            "description": "3 obstacles — repulsive potential for safe avoidance",
        },
        "local_minima": {
            "name": "C. Local Minima",
            "obstacles": [
            ...
            "initial_state": np.array([3.0, 0.0, np.pi / 2]),
            "duration": 15.0,
            "description": "Large obstacle (r=0.8) on path — stagnation detection + repulsive escape",
        },
        "u_shape_trap": {
            "name": "D. U-Shape Trap",
            "obstacles": [
            ...
            "initial_state": np.array([3.0, 0.0, np.pi / 2]),
            "duration": 15.0,
            "description": "U-shape obstacle trap — extreme local minima scenario",
        },
    }
...
def _make_controllers(model, scenario):
    ...
    drpa = DRPAMPPIController(
        model, drpa_params, cost_function=_make_cost(drpa_params, obstacles)
    )
...
def compute_metrics(history, scenario):
    ...
    for info in history["infos"]:
        if isinstance(info, dict) and "drpa_stats" in info:
            escape_count = info["drpa_stats"].get("escape_count", 0)
            escape_steps = sum(
                1 for i2 in history["infos"]
                if isinstance(i2, dict) and "drpa_stats" in i2
                and i2["drpa_stats"].get("in_escape_mode", False)
            )
            escape_ratio = escape_steps / len(history["infos"]) if history["infos"] else 0
            break
...
def run_live(args):
    ...
    print(f"\n{'=' * 60}")
    print(f"  DRPA-MPPI Live -- {scenario['name']}")
    ...
    fig.suptitle(
        f"DRPA-MPPI Live -- {scenario['name']}",
    ...
    # [0,3] Escape Mode (DRPA only)
    ax_esc = axes[0, 3]
    ax_esc.set_xlabel("Time (s)")
    ax_esc.set_ylabel("Escape Mode")
    ax_esc.set_title("DRPA Escape Mode")
    ax_esc.grid(True, alpha=0.3)
    escape_data = []
    line_esc, = ax_esc.plot([], [], color=COLORS["DRPA-MPPI"], linewidth=2)
    ...
    ax_rmse.set_xticklabels(["Van", "DIAL", "Biased", "DRPA"], fontsize=8)
    ...
    ax_safe.set_xticklabels(["Van", "DIAL", "Biased", "DRPA"], fontsize=8)
    ...
    def update(frame):
        ...
        for name, ctrl in controllers.items():
            ...
            # Escape mode (DRPA only)
            if name == "DRPA-MPPI" and "drpa_stats" in info:
                escape_data.append(1.0 if info["drpa_stats"]["in_escape_mode"] else 0.0)
        ...
        if escape_data:
            line_esc.set_data(times[:len(escape_data)], escape_data)
            ax_esc.relim()
            ax_esc.autoscale_view()
            ax_esc.set_ylim(-0.1, 1.1)
    ...
    gif_path = f"plots/drpa_mppi_live_{scenario_key}.gif"
    print(f"\n  Saving GIF ({num_steps} frames) ...")
    anim.save(gif_path, writer="pillow", fps=20, dpi=100)
    print(f"  GIF saved: {gif_path}")

    try:
        mp4_path = f"plots/drpa_mppi_live_{scenario_key}.mp4"
...
def run_benchmark(args):
    ...
    print(f"\n{'=' * 100}")
    print(f"  DRPA-MPPI Benchmark: 4-Way Dynamic Repulsive Potential Comparison")
...
def _plot_results(results, scenario, scenario_key):
    ...
    for r in results:
        ...
        for info in r["infos"]:
            if isinstance(info, dict) and "drpa_stats" in info:
                escape_flags.append(1.0 if info["drpa_stats"]["in_escape_mode"] else 0.0)
        if escape_flags:
            t_plot = np.arange(len(escape_flags)) * dt
            ax.plot(t_plot, escape_flags, color=r["color"], label=r["short"], linewidth=2)
            ax.set_ylim(-0.1, 1.1)
    ax.set_xlabel("Time (s)")
    ax.set_ylabel("Escape Mode (0/1)")
    ax.set_title("DRPA Escape Mode")
    ax.legend(fontsize=7)
    ax.grid(True, alpha=0.3)
    ...
    plt.suptitle(
        f"DRPA-MPPI Benchmark [{scenario_key}]: "
        f"Vanilla vs DIAL vs Biased vs DRPA-MPPI",
        fontsize=13, fontweight="bold",
    )
    plt.tight_layout()

    os.makedirs("plots", exist_ok=True)
    out_path = f"plots/drpa_mppi_benchmark_{scenario_key}.png"
...
def main():
    parser = argparse.ArgumentParser(description="DRPA-MPPI Benchmark")
    parser.add_argument(
        "--scenario", default="simple",
        choices=["simple", "obstacles", "local_minima", "u_shape_trap"],
    )
