#!/usr/bin/env python3
"""
ASR-MPPI (Adaptive Spectral Risk MPPI) 벤치마크: 4-Way x 4 시나리오

방법:
  1. Vanilla MPPI    — 위험 중립
  2. Risk-Aware MPPI — CVaR α=0.7 (경질 절단)
  3. Tsallis-MPPI    — q=0.8 (q-지수 가중)
  4. ASR-MPPI        — sigmoid β=5, α=0.5 (부드러운 스펙트럼 위험)

시나리오 4개:
  A. simple         — 기준선 비교 (장애물 없음)
  B. obstacles      — 3개 원형 장애물
  C. dense_slalom   — 8개 밀집 장애물
  D. adaptive       — 3개 장애물 + 적응 모드

Usage:
    PYTHONPATH=. python examples/comparison/spectral_risk_mppi_benchmark.py --scenario simple
    ...
    PYTHONPATH=. python examples/comparison/spectral_risk_mppi_benchmark.py --no-plot
"""
...
def get_scenarios():
    ...
    return {
        "simple": {
            ...
            "duration": 15.0,
            "description": "No obstacles, circle trajectory — baseline comparison",
            "use_adaptive": False,
        },
        "obstacles": {
            "name": "B. Obstacles",
            ...
            "initial_state": np.array([3.0, 0.0, np.pi / 2]),
            "duration": 15.0,
            "description": "3 circular obstacles — smooth weights advantage",
            "use_adaptive": False,
        },
        "dense_slalom": {
            "name": "C. Dense Slalom",
            ...
            "initial_state": np.array([3.0, 0.0, np.pi / 2]),
            "duration": 15.0,
            "description": "8 dense obstacles (slalom) — SRM expressiveness",
            "use_adaptive": False,
        },
        "adaptive": {
            "name": "D. Adaptive Risk",
            "obstacles": [
                (1.5, 1.5, 0.5),
                (-1.0, 1.0, 0.4),
                (0.5, -1.5, 0.5),
            ...
            "duration": 15.0,
            "description": "3 obstacles + adaptive mode, figure8 — auto adaptation",
            "use_adaptive": True,
        },
    }
...
def _make_controllers(model, scenario):
    ...
    use_adaptive = scenario.get("use_adaptive", False)
    ...
    # Tsallis-MPPI
    t_params = TsallisMPPIParams(**COMMON, tsallis_q=0.8)
    tsallis = TsallisMPPIController(model, t_params)
    tsallis.cost_function = _make_cost(t_params, obstacles)
    ...
    a_params = ASRMPPIParams(**COMMON, **asr_kwargs)
    asr = ASRMPPIController(model, a_params, cost_function=_make_cost(a_params, obstacles))

    return {
        "Vanilla MPPI": vanilla,
        "Risk-Aware MPPI": risk,
        "Tsallis-MPPI": tsallis,
        "ASR-MPPI": asr,
    }
...
def compute_metrics(history, scenario):
    ...
    for info in history["infos"]:
        if isinstance(info, dict):
            # ASR 컨트롤러의 last_info에서 간접 추출 불가
            pass
...
def run_live(args):
    ...
    print(f"\n{'=' * 60}")
    print(f"  ASR-MPPI Live — {scenario['name']}")
    ...
    # [0,3] 왜곡 함수 φ(q) 비교
    ax_phi = axes[0, 3]
    ax_phi.set_xlabel("Quantile q")
    ax_phi.set_ylabel("φ(q)")
    ax_phi.set_title("Distortion Functions")
    ax_phi.grid(True, alpha=0.3)

    q_plot = np.linspace(0, 1, 200)
    # CVaR φ
    alpha_cvar = 0.7
    phi_cvar = np.where(q_plot < 1 - alpha_cvar, 0.0, (q_plot - (1 - alpha_cvar)) / alpha_cvar)
    ax_phi.plot(q_plot, phi_cvar, color=COLORS["Risk-Aware MPPI"], linewidth=2,
                label="CVaR (step)")
    # Sigmoid φ
    alpha_sig, beta_sig = 0.5, 5.0
    sig_raw = 1.0 / (1.0 + np.exp(-beta_sig * (q_plot - alpha_sig)))
    phi_0 = 1.0 / (1.0 + np.exp(-beta_sig * (0.0 - alpha_sig)))
    phi_1 = 1.0 / (1.0 + np.exp(-beta_sig * (1.0 - alpha_sig)))
    phi_sig = (sig_raw - phi_0) / (phi_1 - phi_0)
    ax_phi.plot(q_plot, phi_sig, color=COLORS["ASR-MPPI"], linewidth=2,
                label="Sigmoid (smooth)")
    # Identity (Vanilla)
    ax_phi.plot(q_plot, q_plot, color=COLORS["Vanilla MPPI"], linewidth=1,
    ...
    ax_rmse.set_xticklabels(["Van", "Risk", "Tsallis", "ASR"], fontsize=8)
    bar_rmse_texts = [
    ...
    ax_mc.set_xticklabels(["Van", "Risk", "Tsallis", "ASR"], fontsize=8)
    ...
    def update(frame):
        ...
        ess_vals = []
        for i, name in enumerate(bar_names):
            ess_data = data[name]["ess"]
            mean_ess = np.mean(ess_data) if ess_data else 0
    ...
    gif_path = f"plots/spectral_risk_mppi_live_{scenario_key}.gif"
    print(f"\n  Saving GIF ({num_steps} frames) ...")
    anim.save(gif_path, writer="pillow", fps=20, dpi=100)
    print(f"  GIF saved: {gif_path}")

    try:
        mp4_path = f"plots/spectral_risk_mppi_live_{scenario_key}.mp4"
...
def run_benchmark(args):
    ...
    print(f"\n{'=' * 80}")
    print(f"  ASR-MPPI Benchmark: 4-Way Comparison")
    ...
    for i, (name, ctrl) in enumerate(controllers.items()):
        ...
        # ASR spectral stats
        spectral_risk = 0.0
        if hasattr(ctrl, 'get_risk_statistics'):
            stats = ctrl.get_risk_statistics()
            spectral_risk = stats.get("spectral_risk_value", 0.0)
...
def _plot_results(results, scenario, scenario_key):
    ...
    # Identity (Vanilla)
    ax.plot(q_plot, q_plot, color=COLORS["Vanilla MPPI"], linewidth=1.5,
            linestyle=":", label="Identity (Vanilla)")
    # CVaR
    alpha_cvar = 0.7
    phi_cvar = np.where(q_plot < 1 - alpha_cvar, 0.0,
                        (q_plot - (1 - alpha_cvar)) / alpha_cvar)
    ax.plot(q_plot, phi_cvar, color=COLORS["Risk-Aware MPPI"], linewidth=2,
            label=f"CVaR (α={alpha_cvar})")
    # Sigmoid
    alpha_sig, beta_sig = 0.5, 5.0
    sig_raw = 1.0 / (1.0 + np.exp(-beta_sig * (q_plot - alpha_sig)))
    phi_0 = 1.0 / (1.0 + np.exp(-beta_sig * (0.0 - alpha_sig)))
    phi_1 = 1.0 / (1.0 + np.exp(-beta_sig * (1.0 - alpha_sig)))
    phi_sig = (sig_raw - phi_0) / (phi_1 - phi_0)
    ax.plot(q_plot, phi_sig, color=COLORS["ASR-MPPI"], linewidth=2,
            label=f"Sigmoid (β={beta_sig})")

    ax.set_xlabel("Quantile q")
    ax.set_ylabel("φ(q)")
    ax.set_title("Distortion Functions")
    ax.legend(fontsize=7)
    ax.grid(True, alpha=0.3)

    # (1,0) 가중치 분포 히스토그램 (마지막 스텝)
    ax = axes[1, 0]
    for r in results:
        if r["infos"]:
            last_info = r["infos"][-1]
    ...
    # (1,2) MinClearance 바 차트 (장애물 시나리오)
    ax = axes[1, 2]
    if has_obstacles:
    ...
    plt.suptitle(
        f"ASR-MPPI Benchmark [{scenario_key}]: "
        f"Vanilla vs Risk-Aware vs Tsallis vs ASR-MPPI",
        fontsize=13, fontweight="bold",
    )
    plt.tight_layout()

    os.makedirs("plots", exist_ok=True)
    out_path = f"plots/spectral_risk_mppi_benchmark_{scenario_key}.png"
...
def main():
    parser = argparse.ArgumentParser(description="ASR-MPPI Benchmark")
    parser.add_argument(
        "--scenario", default="simple",
        choices=["simple", "obstacles", "dense_slalom", "adaptive"],
    )
