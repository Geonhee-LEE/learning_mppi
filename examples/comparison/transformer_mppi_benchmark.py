#!/usr/bin/env python3
"""
T-MPPI (Transformer-based MPPI) 벤치마크: 4-Way x 4 시나리오

방법:
  1. Vanilla MPPI    -- 단일 가우시안 (기준선)
  2. DIAL-MPPI       -- 다중 반복 + 어닐링
  3. Biased-MPPI     -- 혼합 분포 (정책 + 가우시안)
  4. T-MPPI          -- Transformer 기반 초기화

시나리오 4개:
  A. simple          -- 기준선 (장애물 없음, circle)
  B. obstacles       -- 3개 원형 장애물
  C. online_learning -- 3개 장애물 + 온라인 학습 모드
  D. warm_start      -- warm start 유무 수렴 비교

Usage:
    PYTHONPATH=. python examples/comparison/transformer_mppi_benchmark.py --scenario simple
    ...
    PYTHONPATH=. python examples/comparison/transformer_mppi_benchmark.py --no-plot
"""
...
def get_scenarios():
    ...
    return {
        ...
        "obstacles": {
            ...
            "initial_state": np.array([3.0, 0.0, np.pi / 2]),
            "duration": 15.0,
            "description": "3 obstacles -- transformer-guided initialization",
        },
        "online_learning": {
            "name": "C. Online Learning",
            "obstacles": [
                (2.5, 1.5, 0.5),
                (0.0, 3.0, 0.4),
                (-2.0, -1.0, 0.5),
            ...
            "initial_state": np.array([3.0, 0.0, np.pi / 2]),
            "duration": 20.0,
            "description": "3 obstacles + online learning -- transformer adapts over time",
        },
        "warm_start": {
            "name": "D. Warm Start Comparison",
            "obstacles": [
                (0.0, 3.0, 0.6),
            ],
            "trajectory_fn": lambda t: circle_trajectory(t, radius=3.0),
            "initial_state": np.array([3.0, 0.0, np.pi / 2]),
            "duration": 15.0,
            "description": "Single large obstacle -- warm start convergence comparison",
        },
    }
...
def _make_controllers(model, scenario):
    ...
    # 4. T-MPPI
    t_params = TransformerMPPIParams(
        **common,
        transformer_hidden_dim=128,
        transformer_n_heads=4,
        transformer_n_layers=2,
        transformer_context_length=20,
        transformer_dropout=0.1,
        transformer_lr=1e-3,
        transformer_buffer_size=5000,
        transformer_min_samples=50,
        transformer_batch_size=32,
        transformer_training_interval=10,
        transformer_n_train_steps=5,
        use_transformer_init=True,
        blend_ratio=0.7,
        online_learning=True,
    )
    t_mppi = TransformerMPPIController(
        model, t_params, cost_function=_make_cost(t_params, obstacles)
    )
...
def compute_metrics(history, scenario):
    ...
    # Transformer init ratio
    transformer_init_ratio = 0.0
    for info in history["infos"]:
        if isinstance(info, dict) and "transformer_stats" in info:
            transformer_init_ratio = info["transformer_stats"].get(
                "transformer_init_ratio", 0.0
            )
    ...
    return {
        ...
        "errors": errors,
        "transformer_init_ratio": transformer_init_ratio,
    }
...
def run_live(args):
    ...
    print(f"\n{'=' * 60}")
    print(f"  T-MPPI Live -- {scenario['name']}")
    ...
    # [0,3] Transformer Init Usage
    ax_tinit = axes[0, 3]
    ax_tinit.set_xlabel("Time (s)")
    ax_tinit.set_ylabel("Init Ratio")
    ax_tinit.set_title("T-MPPI Transformer Init Ratio")
    ax_tinit.grid(True, alpha=0.3)
    tinit_data = []
    line_tinit, = ax_tinit.plot([], [], color=COLORS["T-MPPI"], linewidth=2)
    ...
    ax_rmse.set_xticklabels(["Van", "DIAL", "Biased", "T-MPPI"], fontsize=8)
    ...
    ax_safe.set_xticklabels(["Van", "DIAL", "Biased", "T-MPPI"], fontsize=8)
    ...
    def update(frame):
        ...
        for name, ctrl in controllers.items():
            ...
            # Transformer init ratio (T-MPPI only)
            if name == "T-MPPI" and "transformer_stats" in info:
                tinit_data.append(info["transformer_stats"]["transformer_init_ratio"])
        ...
        if tinit_data:
            line_tinit.set_data(times[:len(tinit_data)], tinit_data)
            ax_tinit.relim()
            ax_tinit.autoscale_view()
            ax_tinit.set_ylim(-0.05, 1.05)
        ...
        for i, name in enumerate(bar_names):
            ...
            rmse_val = np.sqrt(np.mean(np.array(errs) ** 2)) if errs else 0
            rmses.append(rmse_val)
            bars_rmse[i].set_height(rmse_val)
        if rmses:
        ...
        for name in controllers:
            ...
            lines.append(f"{short:>8}: RMSE={rmse_val:.3f} ESS={ess:.0f}")
        info_text.set_text("\n".join(lines))
    ...
    gif_path = f"plots/transformer_mppi_live_{scenario_key}.gif"
    print(f"\n  Saving GIF ({num_steps} frames) ...")
    anim.save(gif_path, writer="pillow", fps=20, dpi=100)
    print(f"  GIF saved: {gif_path}")

    try:
        mp4_path = f"plots/transformer_mppi_live_{scenario_key}.mp4"
    ...
    for name in controllers:
        ...
        line = f"  {name:<18} {rmse_val:>8.4f} {mean_ess:>10.1f}"
        if has_obstacles:
            c = clearance_data[name]
...
def run_benchmark(args):
    ...
    print(f"\n{'=' * 100}")
    print(f"  T-MPPI Benchmark: 4-Way Transformer Initialization Comparison")
    ...
    for r in all_results:
        line = (
            ...
            f"{r['transformer_init_ratio']:>12.3f}"
        )
        if has_obstacles:
...
def _plot_results(results, scenario, scenario_key):
    ...
    for r in results:
        ...
        for info in r["infos"]:
            if isinstance(info, dict) and "transformer_stats" in info:
                tinit_flags.append(
                    info["transformer_stats"].get("transformer_init_ratio", 0.0)
                )
        if tinit_flags:
            t_plot = np.arange(len(tinit_flags)) * dt
            ax.plot(t_plot, tinit_flags, color=r["color"], label=r["short"], linewidth=2)
    ax.set_xlabel("Time (s)")
    ax.set_ylabel("Init Ratio")
    ax.set_title("Transformer Init Ratio (T-MPPI)")
    ax.legend(fontsize=7)
    ax.grid(True, alpha=0.3)
    ...
    for r in results:
        ...
        if r["transformer_init_ratio"] > 0:
            line_txt += f"  TfInit={r['transformer_init_ratio']:.1%}"
    ...
    plt.suptitle(
        f"T-MPPI Benchmark [{scenario_key}]: "
        f"Vanilla vs DIAL vs Biased vs T-MPPI",
        fontsize=13, fontweight="bold",
    )
    plt.tight_layout()

    os.makedirs("plots", exist_ok=True)
    out_path = f"plots/transformer_mppi_benchmark_{scenario_key}.png"
...
def main():
    parser = argparse.ArgumentParser(description="T-MPPI Benchmark")
    parser.add_argument(
        "--scenario", default="simple",
        choices=["simple", "obstacles", "online_learning", "warm_start"],
    )
