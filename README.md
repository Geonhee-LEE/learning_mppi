...
[![Tests](https://img.shields.io/badge/Tests-1793%2B%20Passing-brightgreen)](tests/)

A comprehensive MPPI (Model Predictive Path Integral) control library featuring 43 SOTA variants, **22 safety-critical control methods**, 14 learning models, 5 robot model types, GPU acceleration, learning-based dynamics, MAML meta-learning, post-MAML adaptation (EKF/L1/ALPaCA), **Conformal Prediction + CBF** for distribution-free dynamic safety margins, **Evidential Deep Learning (EDL)** for single-pass aleatoric/epistemic uncertainty separation, **SimulationHarness** for unified multi-controller comparison, **robot body rendering** (circle/car/rectangle patches), and **safety visualization overlay** (CBF contour/collision cone).
...
| 38 | **Koopman-MPPI** | Koopman (1931) / Brunton et al. (2016) | EDMD-based linear Koopman dynamics for accelerated batch rollout |
| 39 | **World Model MPPI** | Hafner et al. (Dreamer family) | Rollout in an RSSM/Linear latent space — Latent-MPPI |
| 40 | **PGD-MPPI** | arXiv:2603.24489, 2026 | MPPI as preconditioned gradient descent — generalizes step size / multi-step / covariance preconditioning |
| 41 | **TR-MPPI** | arXiv:2605.07801, 2026 | Trust-region (KL-bounded) proposal update + Halton LCD deterministic sampling |
| 42 | **RF-MPPI** | arXiv:2511.19204, 2026 | Reference-free dual-space cubic Hermite spline — high smoothness with few samples |
| 43 | **Step-MPPI** | arXiv:2604.01539, 2026 | Learns proposal distribution (mean + covariance) via DPC — single-step lookahead, ultra-low latency |
...
### MPPI Algorithm Improvements
- [ ] **KMPPI (Kernel MPPI)** — few support points + RBF kernel interpolation for 75% lower sampling dimension + 2x smoother control (ref: [pytorch_mppi](https://github.com/UM-ARM-Lab/pytorch_mppi))
- [ ] **Pure PyTorch Tensor Pipeline** — transparent CPU/GPU switching, diagonal-covariance optimization, torch.compile support (ref: [pytorch_mppi](https://github.com/UM-ARM-Lab/pytorch_mppi))
- [ ] **Topology-Aware MPPI** — parallel search over multiple homotopy classes to escape local minima (ref: [mpc_planner](https://github.com/tud-amr/mpc_planner) T-MPC++)
- [ ] **Parameter-Robust MPPI** — SVGD-based controller parameter optimization + dual control (exploration vs. exploitation)
- [ ] **MPPI Autotune** — automatic tuning of lambda, sigma, horizon via CMA-ES/Ray Tune (ref: [pytorch_mppi](https://github.com/UM-ARM-Lab/pytorch_mppi))
- [ ] **MPPI Batched** — process N environments x K samples in a single batch for multi-agent/multi-sim parallelism
- [ ] **Latent-Space MPPI** — MPPI planning in a World Model (VAE/Transformer) latent space
- [ ] **Decentralized Multi-Agent MPPI** — SOCP-based distributed MPPI for cooperative multi-robot control

### Learning Model Extensions
- [x] **Evidential Deep Learning (EDL)** — single-pass epistemic/aleatoric uncertainty separation (10x faster than ensembles)
- [ ] **Learnable Conformal Prediction** — context-aware nonconformity function improving safety margin 72→91%
- [x] **BNN-MPPI** — ensemble-uncertainty feasibility filtering via a BNN surrogate model (16th variant)
- [ ] **Neural Safety Verifier** — neural-network safety verification via SDP/CROWN abstractions

### Simulation & Visualization
- [ ] **Limited-FOV Sensing Simulation** — field-of-view/range-limited sensor model, cost only over detected obstacles (ref: [safe_control](https://github.com/tkkim-robot/safe_control))
- [ ] **Superellipsoid Obstacles** — non-circular obstacle representation (rectangle/ellipse approximation, differentiable) (ref: [safe_control](https://github.com/tkkim-robot/safe_control))
- [ ] **Sample Trajectory Visualization** — cost-based transparency rendering + control-input gauges (ref: [python_simple_mppi](https://github.com/MizuhoAOKI/python_simple_mppi))
- [ ] **Crowd Navigation** — Social Force + pedestrian trajectory prediction (SocialLSTM) integration
- [ ] **Classic Control Demos** — Pendulum Swing-Up, CartPole stabilization to demonstrate MPPI generality (ref: [python_simple_mppi](https://github.com/MizuhoAOKI/python_simple_mppi))
- [ ] **Jupyter Notebook Tutorials** — interactive step-by-step demos per key variant (Vanilla/Flow/C2U)
- [ ] **Ablation Study Framework** — parameter sweep + auto-visualization benchmarking framework

### Robot Model Extensions
- [ ] **Dynamic Bicycle (Fiala Tire)** — longitudinal/lateral slip modeling, drift simulation (ref: [safe_control](https://github.com/tkkim-robot/safe_control))
- [ ] **Quadrotor 3D** — 12-state 6-DOF UAV + RK4 Sampled Data CBF (ref: [safe_control](https://github.com/tkkim-robot/safe_control))
- [ ] **Mecanum Drive** — omnidirectional motion with slip modeling (Kinematic + Dynamic)
- [ ] **Single/Double Integrator** — 2D/4D basic models for CBF teaching (ref: [safe_control](https://github.com/tkkim-robot/safe_control))
- [ ] **Multi-Disc Robot** — approximate large robots as multiple discs for precise collision avoidance (ref: [mpc_planner](https://github.com/tud-amr/mpc_planner))
